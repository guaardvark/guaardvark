...
class TranscriptParser:
    ...
    def __init__(self, output_dir: str = None):
        self.output_dir = Path(
            output_dir
            or os.path.join(
                os.environ.get("GUAARDVARK_ROOT", "."), "training", "processed"
            )
        )
    ...
    def _parse_text(self, content: str) -> List[Dict]:
        ...
        for human_pattern, ai_pattern in patterns:
            human_matches = re.findall(
                human_pattern, content, re.DOTALL | re.IGNORECASE
            )
            ...
            if human_matches and ai_matches:
                for h, a in zip(human_matches, ai_matches):
                    ...
                    if h_clean and a_clean and len(h_clean) > 5 and len(a_clean) > 10:
                        pairs.append({"instruction": h_clean, "output": a_clean})
                if pairs:
                    break
        ...
        for msg in messages:
            ...
            if role in ["user", "human"]:
                ...
                if len(user_msg) > 5 and len(content) > 10:
                    pairs.append({"instruction": user_msg, "output": content})
                user_msg = None
    ...
    def create_alpaca_format(self, input_jsonl: str, output_file: str = None) -> str:
        ...
        output_path = (
            Path(output_file) if output_file else input_path.with_suffix(".alpaca.json")
        )
...
def main():
    ...
    parser.add_argument(
        "--alpaca", action="store_true", help="Also create Alpaca format"
    )
