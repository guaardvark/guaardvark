...
def initialize_model(
    model_name: Optional[str] = None, ollama_base_url: str = "http://localhost:11434"
) -> bool:
    """
    ...
    try:
        ...
        if os.environ.get("CUDA_VISIBLE_DEVICES", "").strip() == "":
            ...
            logger.info(
                f"Using GPU device: {os.environ.get('CUDA_VISIBLE_DEVICES', '0')}"
            )
        ...
        if not model_name:
            ...
            if use_system_model:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not import get_active_embedding_model: {e}, using plugin config"
                    )
                    model_name = config.get("model", "embeddinggemma:latest")
                except Exception as e:
                    logger.warning(
                        f"Could not get active embedding model: {e}, using plugin config"
                    )
                    model_name = config.get("model", "embeddinggemma:latest")
            else:
...
def get_model_info() -> Dict[str, Any]:
    ...
    info = {"model_name": _model_name, "loaded": _model_loaded, "embed_dim": None}
