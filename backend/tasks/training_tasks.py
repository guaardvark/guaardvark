            ...
            if progress_system:
                ...
                if status == "complete":
                    progress_system.complete_process(
                        job_id, message, {"job_id": job_id, "metrics": metrics or {}}
                    )
                elif status == "error":
                    ...
                    progress_system.cancel_process(
                        job_id, message, {"job_id": job_id, "metrics": metrics or {}}
                    )
                else:
                    progress_system.update_process(
                        ...
                        progress,
                        message,
                        {"job_id": job_id, "metrics": metrics or {}},
                    )
    except Exception as e:
...
def parse_transcripts_task(self, job_id: str, input_path: str, recursive: bool = True):
    ...
    try:
        ...
        _emit_progress(
            job_id, 10, f"Parsing transcripts from {input_path}...", "processing"
        )
                ...
                if file_path.is_file() and file_path.suffix in [
                    ...
                    _emit_progress(
                        ...
                        + int(
                            80
                            * len(pairs)
                            / max(1, len(list(input_path_obj.glob(pattern))))
                        ),
        ...
        output_filename = (
            f"training_corpus_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
        )
        output_path = PROCESSED_DIR / output_filename

        _emit_progress(
            job_id, 90, f"Saving {len(pairs)} training pairs...", "processing"
        )
        ...
        _emit_progress(
            job_id,
            100,
            f"Completed: {len(pairs)} training pairs saved to {output_filename}",
            "complete",
        )
...
def filter_dataset_task(self, job_id: str, input_path: str, min_score: float = 0.5):
    ...
    try:
        ...
        _emit_progress(
            job_id, 10, f"Loading dataset from {input_path}...", "processing"
        )
        ...
        _emit_progress(
            job_id,
            30,
            f"Filtering {len(pairs)} pairs (min_score={min_score})...",
            "processing",
        )
        ...
        for i, pair in enumerate(pairs):
            ...
            if (i + 1) % 100 == 0:
                _emit_progress(
                    job_id,
                    30 + int(50 * (i + 1) / len(pairs)),
                    f"Filtered {len(filtered_pairs)}/{i+1} pairs...",
                    "processing",
                )

        output_filename = (
            f"filtered_dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
        )
        output_path = PROCESSED_DIR / output_filename

        _emit_progress(
            job_id, 90, f"Saving {len(filtered_pairs)} filtered pairs...", "processing"
        )
        ...
        logger.info(
            f"Filter task completed for job {job_id}: {len(filtered_pairs)}/{len(pairs)} pairs"
        )
        return {
            "output_path": str(output_path),
            "filtered_count": len(filtered_pairs),
...
@shared_task(
    bind=True, name="training.finetune_model", soft_time_limit=86400, time_limit=172800
)
def finetune_model_task(self, job_id: str, config: dict, resume: bool = False):
    ...
    try:
        ...
        output_name = (
            ...
            or f"guaardvark-{base_model.replace('/', '-').replace(':', '-')}"
        )
        max_steps = job_config.get("steps", 500)
        learning_rate = job_config.get("lr", 2e-4)
        batch_size = job_config.get(
            "batch_size", device_profile.max_batch_size if device_profile else 2
        )
        lora_rank = job_config.get("rank", 16)
        max_seq_length = job_config.get(
            "seq_length", device_profile.max_seq_length if device_profile else 2048
        )
        offload_to_cpu = job_config.get(
            "cpu_offload",
            device_profile.requires_cpu_offload if device_profile else False,
        )
        ...
        if images_path:
            _emit_progress(
                job_id,
                8,
                f"Detected vision task. Using Qwen-VL trainer with images from {images_path}",
                "processing",
            )
            ...
            _emit_progress(
                job_id,
                10,
                f"Starting vision training loop{resume_msg}...",
                "processing",
            )
            model_dir = finetune(
            ...
            resume_msg = " (resuming from checkpoint)" if resume else ""
            _emit_progress(
                job_id, 10, f"Starting text training loop{resume_msg}...", "processing"
            )
            model_dir = finetune(
                base_model=base_model,
                data_path=data_path,
        ...
        _emit_progress(
            job_id, 100, f"Training complete! Model saved to {model_dir}", "complete"
        )
        ...
        try:
            output_name = (
                ...
                or f"guaardvark-{config.get('base_model', 'model').replace('/', '-')}"
            )
...
def export_gguf_task(self, job_id: str, model_dir: str, quantization: str = "q4_k_m"):
    ...
    try:
        ...
        _emit_progress(
            job_id, 20, f"Exporting to GGUF ({quantization})...", "processing"
        )
        ...
        _update_job_status(
            job_id, pipeline_stage="exporting", progress=100, gguf_path=str(gguf_path)
        )
...
def _generate_modelfile_from_template(
    template_name: str, gguf_path: Path, mmproj_path: Path = None
) -> str:
...
def import_ollama_task(self, job_id: str, model_dir: str, model_name: str):
    ...
    try:
        ...
        _emit_progress(
            job_id, 20, f"Creating Modelfile for {model_name}...", "processing"
        )
        ...
        modelfile_content = _generate_modelfile_from_template(
            template_name, main_gguf, mmproj_gguf
        )
...
@shared_task(
    bind=True, name="training.full_pipeline", soft_time_limit=259200, time_limit=345600
)
def full_training_pipeline_task(self, job_id: str, config: dict):
    ...
    try:
        ...
        _emit_progress(
            job_id, 1, "Freeing GPU memory (unloading Ollama models)...", "processing"
        )
        try:
            from backend.services.gpu_resource_coordinator import (
            ...
            logger.info(
                f"Initial VRAM: {initial_vram.get('available_mb', 'unknown')} MB available"
            )
            ...
            if unload_result.get("success"):
                ...
                if models_unloaded:
                    logger.info(
                        f"Unloaded Ollama models: {models_unloaded}, freed {vram_freed} MB"
                    )
                    _emit_progress(
                        job_id, 2, f"Freed {vram_freed} MB GPU memory", "processing"
                    )
                else:
                    logger.info("No Ollama models were loaded")
            else:
                logger.warning(
                    f"Failed to unload Ollama models: {unload_result.get('error')}"
                )
        except Exception as e:
        ...
        if job_config.get("input_path"):
            ...
            parse_result = parse_transcripts_task(
                job_id, job_config["input_path"], job_config.get("recursive", True)
            )
        ...
        if job_config.get("min_score") is not None and filter_output_path:
            ...
            filter_result = filter_dataset_task(
                job_id, filter_output_path, job_config.get("min_score", 0.5)
            )
        ...
        export_result = export_gguf_task(
            job_id, model_dir, job_config.get("quantization", "q4_k_m")
        )
        ...
        if not model_name:
            model_name = (
                f"guaardvark-{job.base_model.replace('/', '-').replace(':', '-')}"
            )
        ...
        return {
            ...
            "filter_output": (
                filter_output_path if job_config.get("min_score") is not None else None
            ),
            "model_dir": model_dir,
