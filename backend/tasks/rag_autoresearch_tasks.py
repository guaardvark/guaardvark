...
def create_autoresearch_tasks(celery_app):
    ...
    def check_idle_and_start():
        ...
        try:
            ...
            idle_setting = Setting.query.filter_by(
                key="rag_autoresearch_idle_minutes"
            ).first()
            idle_minutes = int(idle_setting.value) if idle_setting else 10

            auto_setting = Setting.query.filter_by(
                key="rag_autoresearch_auto_enabled"
            ).first()
            auto_enabled = (
                (auto_setting.value.lower() == "true") if auto_setting else True
            )
            ...
            if svc.is_idle(idle_minutes=idle_minutes):
                max_setting = Setting.query.filter_by(
                    key="rag_autoresearch_max_experiments"
                ).first()
                max_exp = (
                    int(max_setting.value)
                    if max_setting and max_setting.value != "0"
                    else 0
                )
    ...
    def on_index_complete():
        ...
        try:
            ...
            if svc.eval_harness.is_stale():
                logger.info(
                    "Eval set is stale after indexing — will regenerate on next run"
                )
        except Exception as e:
