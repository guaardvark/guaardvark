...
def generate_proven_csv_task(
    ...
    try:
        logger.info(
            f"Starting proven CSV generation: {len(topics)} topics for {client_name}"
        )
        ...
        try:
            ...
            logger.info(
                f"Progress tracking initialized: progress_id={progress_id}, created_id={created_id}"
            )
        ...
        def get_category_from_topic(topic):
            ...
            if any(
                term in topic_lower
                for term in ["actos", "drug", "medication", "pharmaceutical"]
            ):
                return "Pharmaceutical Law"
            elif any(
                term in topic_lower
                for term in ["malpractice", "medical", "doctor", "hospital"]
            ):
                return "Medical Malpractice"
            elif any(
                term in topic_lower for term in ["wrongful death", "death", "fatality"]
            ):
                return "Wrongful Death"
            elif any(
                term in topic_lower for term in ["birth", "cerebral palsy", "delivery"]
            ):
                return "Birth Injury"
            elif any(
                term in topic_lower for term in ["work", "compensation", "workplace"]
            ):
            ...
            try:
                messages = [
                    ChatMessage(
                        ...
                        content="You are a legal content writer. Write detailed HTML content for law firms. Respond with only the HTML content.",
                    ),
        ...
        if progress_system and progress_id:
            progress_system.update_process(
                progress_id, 0, "Starting content generation..."
            )
        ...
        for i in range(0, len(topics), batch_size):
            ...
            for j, topic in enumerate(batch):
                ...
                try:
                    ...
                    logger.info(
                        f"Generated {row.id}: {topic[:30]}... ({word_count} words)"
                    )
                except Exception as e:
        ...
        stats = {
            ...
            "avg_words_per_page": (
                total_words // len(generated_rows) if generated_rows else 0
            ),
        ...
        if progress_system and progress_id:
            progress_system.update_process(
                progress_id, 100, f"Complete! Generated {len(generated_rows)} pages"
            )

        logger.info(
            f"COMPLETE: {output_path} - {len(generated_rows)} pages, {total_words:,} words"
        )
