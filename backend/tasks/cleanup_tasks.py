...
def create_cleanup_tasks(celery_app: Celery):
    ...
    def cleanup_old_chat_sessions(max_age_days: int = 30, max_sessions: int = 1000):
        ...
        logger.info(
            f"Starting background cleanup of chat sessions (max_age_days={max_age_days}, max_sessions={max_sessions})"
        )
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.error(
                    f"Background cleanup failed: {result.get('error', 'Unknown error')}"
                )
    ...
    def cleanup_user_behavior_log_task(max_size_mb: int = 100, max_age_days: int = 90):
        ...
        logger.info(
            f"Starting background cleanup of user behavior log (max_size_mb={max_size_mb}, max_age_days={max_age_days})"
        )
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.info(
                    f"Behavior log cleanup: {result.get('message', 'No action needed')}"
                )
            ...
            logger.error(
                f"Background behavior log cleanup task failed: {e}", exc_info=True
            )
            return {"error": str(e)}

    @celery_app.task(name="cleanup_progress_jobs")
    def cleanup_progress_jobs_task(
        ...
        logger.info(
            f"Starting background cleanup of progress jobs (max_age_hours={max_age_hours}, clean_completed={clean_completed})"
        )
        ...
        try:
            ...
            if clean_completed:
                cmd.extend(
                    ["--clean-completed", "--max-age-days", str(max_completed_age_days)]
                )
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"Background progress jobs cleanup completed: {cleaned_count} orphaned jobs removed"
                )
                return {
                    "success": True,
            ...
            logger.error(
                f"Background progress jobs cleanup task failed: {e}", exc_info=True
            )
            return {"error": str(e)}
    ...
    def periodic_comprehensive_cleanup():
        ...
        results = {
            "session_cleanup": cleanup_old_chat_sessions.delay(
                max_age_days=7, max_sessions=500
            ),
            "behavior_log_cleanup": cleanup_user_behavior_log_task.delay(
                max_size_mb=50, max_age_days=30
            ),
            "progress_jobs_cleanup": cleanup_progress_jobs_task.delay(
                max_age_hours=24, clean_completed=True, max_completed_age_days=7
            ),
        }

        return {
            "status": "comprehensive_cleanup_tasks_scheduled",
            "task_ids": [str(r.id) for r in results.values()],
        }
    ...
    def periodic_chat_cleanup():
        ...
        return {
            "status": "cleanup_tasks_scheduled",
            "task_ids": [str(r.id) for r in results.values()],
        }
        ...
        **celery_app.conf.beat_schedule,
        ...
        "cleanup-progress-jobs": {
            ...
            "args": (
                ...
                False,
                7,
            ),  # 24 hours for stuck jobs, don't clean completed jobs, 7 days for completed
        },
        ...
        "periodic-chat-cleanup": {
            ...
            ),  # Weekly on Sunday at 4 AM (legacy)
        },
    }
