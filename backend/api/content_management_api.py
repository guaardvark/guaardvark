...
def list_pages():
    ...
    try:
        ...
        for page in pages:
            ...
            generation = (
                db.session.query(Generation).filter_by(id=page.generation_id).first()
            )
            ...
            pages_data.append(
                {
                    ...
                    "status": page.status,
                    "approved_at": (
                        page.approved_at.isoformat() if page.approved_at else None
                    ),
                    "meta": meta,
        ...
        return jsonify(
            {
                ...
                "generation": {
                    ...
                    "created_at": (
                        ...
                        if generation and generation.created_at
                        else None
                    ),
...
def mark_not_uploaded(page_id):
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "page": {"id": page.id, "title": page.title, "uploaded_to_wp": False},
            }
        )
    ...
    try:
        ...
        if page_ids:
            existing_by_id = (
                ...
                .join(Generation)
                .filter(Page.id.in_(page_ids), Generation.site_key.like(f"%{website}%"))
                .all()
            )
        ...
        if slugs:
            existing_by_slug = (
                db.session.query(Page)
                .join(Generation)
                .filter(Page.slug.in_(slugs), Generation.site_key.like(f"%{website}%"))
                .all()
            )
...
def get_stats():
    ...
    try:
        ...
        if website:
            pages_query = pages_query.join(Generation).filter(
                Generation.site_key.like(f"%{website}%")
            )
        ...
        uploaded_pages = pages_query.filter(
            Page.meta_json.like('%"uploaded_to_wp":true%')
        ).count()
        ...
        recent_generations = generations_query.filter(
        ...
        return jsonify(
            {
                ...
                "upload_rate": (
                    (uploaded_pages / total_pages * 100) if total_pages > 0 else 0
                ),
            }
        )
...
def delete_page(page_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Page {page_id} marked as deleted"}
        )
