...
def test_image_generation_progress():
    ...
    try:
        ...
        process_id = progress_system.create_process(
            ProcessType.IMAGE_GENERATION, "Test Image Generation", {"test": True}
        )

        def simulate_progress():
            for i in range(0, 101, 10):
                progress_system.update_process(
                    process_id, i, f"Generating image... {i}% complete"
                )
                time.sleep(0.5)
...
def test_file_generation_progress():
    ...
    try:
        progress_system = get_unified_progress()
        process_id = progress_system.create_process(
            ProcessType.FILE_GENERATION, "Test File Generation", {"test": True}
        )

        def simulate_progress():
            progress_system.update_process(
                process_id, 10, "Initializing file generation..."
            )
            time.sleep(0.5)
...
def test_indexing_progress():
    ...
    try:
        progress_system = get_unified_progress()
        process_id = progress_system.create_process(
            ProcessType.INDEXING, "Test Document Indexing", {"test": True}
        )
...
def test_analysis_progress():
    ...
    try:
        progress_system = get_unified_progress()
        process_id = progress_system.create_process(
            ProcessType.ANALYSIS, "Test Code Analysis", {"test": True}
        )

        def simulate_progress():
            progress_system.update_process(
                process_id, 15, "Analyzing code structure..."
            )
            time.sleep(0.5)
...
def test_upload_progress():
    ...
    try:
        progress_system = get_unified_progress()
        process_id = progress_system.create_process(
            ProcessType.UPLOAD, "Test File Upload", {"test": True}
        )
...
def test_socketio_connection():
    ...
    try:
        progress_system = get_unified_progress()
        process_id = progress_system.create_process(
            ProcessType.CSV_PROCESSING, "SocketIO Test", {"test": True}
        )
