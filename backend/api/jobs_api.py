...
def detect_stuck_jobs(
    jobs: List[Dict[str, Any]], active_celery_tasks: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        for task_id, task_info in active_celery_tasks.items():
            ...
            if (
                task_id in training_job_celery_ids
                and training_job_celery_ids[task_id] == job_id
            ):
        ...
        if not has_celery_task and is_stale:
            ...
            if is_stale:
                hours_stale = (
                    (current_time - last_update).total_seconds() / 3600
                    if last_update
                    else 999
                )
                    ...
                    with open(metadata_file, "r", encoding="utf-8") as f:
                        ...
                        job_data = {
                            ...
                            "progress": metadata.get(
                                "progress", metadata.get("processed_item_count", 0)
                            ),
                            ...
                            "timestamp": metadata.get(
                                "last_update_utc"
                            ),  # For DevToolsPage compatibility
                            ...
                            "model": metadata.get(
                                "script_generating_model_name_version"
                            ),
...
def get_active_jobs_route():
    ...
    try:
                        ...
                        training_jobs_in_progress.append(
                            {
                                ...
                                "last_update": (
                                    tj.started_at.isoformat() if tj.started_at else None
                                ),
                                "last_update_utc": (
                                    tj.started_at.isoformat() if tj.started_at else None
                                ),
        ...
        if stuck_jobs:
            logger.warning(
                f"Detected {len(stuck_jobs)} stuck jobs: {[job.get('job_id', 'unknown') for job in stuck_jobs]}"
            )
        ...
        real_active_jobs = [
            job for job in all_active_jobs if job.get("job_id") not in stuck_ids
        ]
...
def cleanup_stuck_jobs_route():
    ...
    try:
        ...
        for stuck_job in stuck_jobs:
            ...
            is_training_job = (
                stuck_job.get("additional_data", {}).get("training_job_id") is not None
            )
                ...
                for tj in completed_training_jobs:
                    ...
                    if job_dir.exists() and job_dir.is_dir():
                        try:
                            ...
                            cleaned_count += 1
                            logger.debug(
                                f"Cleaned up old completed training job: {tj.job_id}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Could not clean up old job {tj.job_id}: {e}"
                            )
            except Exception as e:
...
def retry_job_route(job_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": f"Job {job_id} retry initiated.", "new_job_id": new_job_id}
            ),
            200,
        )
    except Exception as e:
