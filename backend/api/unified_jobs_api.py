...
def create_unified_job():
    ...
    try:
        ...
        process_type = (
            ...
            else ProcessType.TASK_PROCESSING
        )

        job_id = progress_system.create_process(
            process_type, f"Creating {data['type']} job: {data['name']}"
        )
        ...
        progress_system.update_process(
            job_id, 10, f"Task {task_id} created, routing to {execution_type}"
        )

        # Route to appropriate backend system based on execution type
        result = route_job_execution(
            task_id, job_id, execution_type, workflow_config, data
        )
...
def route_job_execution(
    task_id, job_id, execution_type, workflow_config, original_data
):
    """
    ...
    try:
        if execution_type == "bulk_csv_generation":
            ...
            csv_config = {
                ...
                "project": workflow_config.get("context_variables", {}).get(
                    "project", "Project"
                ),
                "website": workflow_config.get("context_variables", {}).get(
                    "website", "website.com"
                ),
            ...
            try:
                ...
                if task:
                    ...
                    db.session.commit()
                    logger.info(
                        f"Updated task {task_id} with output filename: {csv_config['output_filename']}"
                    )
            ...
            celery_task = generate_bulk_csv_v2_task.apply_async(
                (csv_config, job_id), queue="generation"
            )

            logger.info(
                f"Submitted bulk CSV generation to Celery: task_id={celery_task.id}"
            )
            ...
            return {"routed_to": "website_analysis", "status": "not_implemented"}

        elif execution_type == "sequential_tasks":
            # Route to sequential task processing
            # TODO: Implement sequential processing routing
            return {"routed_to": "sequential_tasks", "status": "not_implemented"}

        else:
            # Standard task processing
            return {"routed_to": "standard_task", "status": "queued_for_processing"}
            ...
            return jsonify(
                {
                    ...
                    "progress": (
                        {
                            ...
                            "message": (
                                progress.message if progress else "No progress data"
                            ),
                            "last_update": (
                                progress.timestamp.isoformat()
                                if progress and progress.timestamp
                                else None
                            ),
                        }
                        if progress
                    ...
                    "workflow_config": (
                        json.loads(task.workflow_config)
