...
unified_gen_bp = Blueprint(
    "unified_generation_api", __name__, url_prefix="/api/generate"
)
    ...
    try:
        ...
        if generation_type == "single":
            ...
            return format_validation_error(
                f"Invalid generation type: {generation_type}"
            )
...
def _detect_generation_type(data: Dict) -> str:
    ...
    if (
        data.get("topics")
        and isinstance(data["topics"], list)
        and len(data["topics"]) > 1
    ):
        return "bulk"
    ...
    try:
        ...
        if not output_dir:
            return format_generation_error(
                "Server configuration error: Output directory not set", status_code=500
            )
        ...
        output_path, stats = generator.generate_bulk_csv(
            tasks=[task], output_filename=secure_filename_result
        )
...
def generate_large_scale_csv():
    ...
    try:
        ...
        tasks = create_tasks_from_topics(
            topics=topics, client=client, project=project, website=website
        )

        logger.info(
            f"Starting large-scale CSV generation: {len(tasks)} tasks with web research: {enable_web_research}"
        )
        ...
        response_data = {
            ...
            "statistics": {
                **stats,
                ...
                "total_chunks": (
                    len(tasks) // chunk_size + (1 if len(tasks) % chunk_size else 0)
                    if num_items > 500
                    else 1
                ),
                "chunk_size": chunk_size,
