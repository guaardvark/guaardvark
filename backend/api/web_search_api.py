...
def enhanced_web_search(query: str) -> Dict[str, Any]:
    ...
    results = {"query": query, "strategy_used": "", "success": False, "data": {}}
    ...
    return {
        ...
        "data": {
            ...
            "errors": results["data"],
            "attempted_strategies": ["duckduckgo_search"]
            + (["direct_website"] if urls else []),
        },
    }
...
def handle_special_queries(query: str) -> Dict[str, Any]:
    ...
    if any(
        ...
        for keyword in ["time", "clock", "current time", "what time"]
    ):
        try:
        ...
        try:
            ...
            for pattern in location_patterns:
                ...
                if match:
                    ...
                    location = re.sub(
                        r"\b(right\s+now|now|currently|today|tonight)\b",
                        "",
                        location,
                        flags=re.IGNORECASE,
                    ).strip()
            ...
            if not location:
                ...
                if len(words) >= 2:
                    ...
                    location = re.sub(
                        r"\b(what|is|the|current)\b", "", location
                    ).strip()
            ...
            if location and len(location) > 1:
                ...
                if weather_result.get("success"):
                    ...
                    logger.warning(
                        f"Weather lookup failed for {location}: {weather_result.get('error', 'Unknown error')}"
                    )
            else:
                logger.warning(
                    f"Could not extract location from weather query: {query}"
                )
        except Exception as e:
            logger.warning(f"Weather query processing failed: {e}")

    if any(
        keyword in query_lower for keyword in ["calculate", "math", "equation", "="]
    ) and any(op in query for op in ["+", "-", "*", "/", "="]):
        try:
            import re
    ...
    return {"query": query, "success": False, "strategy_used": "none"}
    ...
    try:
        if not get_web_access():
            return error_response(
                "Web search is disabled in system settings", status_code=403
            )
        ...
        if search_results["success"]:
            result = {
                ...
                "source": search_results["data"].get(
                    "source", search_results["strategy_used"]
                ),
            ...
            logger.info(
                f"Enhanced quick search successful using {search_results['strategy_used']}"
            )
            ...
            result = {
                ...
                "attempted_strategies": search_results["data"].get(
        ...
        return success_response(
            {
                ...
                "services": services,
                "timestamp": datetime.now().isoformat(),
                "search_strategies": ["direct_website", "duckduckgo_search"],
                "reliability_notes": {
