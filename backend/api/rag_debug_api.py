...
class RAGDebugManager:
    ...
    def __init__(self):
        self.index_manager = (
            get_global_index_manager() if get_global_index_manager else None
        )
    ...
    def _calculate_relevance_score(self, query: str, chunks: List[str]) -> float:
        ...
        for chunk in chunks:
            ...
            if chunk_words:
                relevance = len(query_words & chunk_words) / len(
    ...
    def _calculate_freshness_score(self, nodes: List[Any]) -> float:
        ...
        return (
            sum(freshness_scores) / len(freshness_scores) if freshness_scores else 0.5
        )

    def debug_retrieval(
        self, query: str, project_id: Optional[str] = None, top_k: int = 5
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                retriever = self.index_manager.get_retriever(
                    project_id=project_id, similarity_top_k=top_k
                )
            ...
            try:
                ...
                logger.error(
                    f"BUG FIX 2: LlamaIndex AssertionError caught during retrieval: {e}"
                )
            ...
            self.system_metrics["avg_retrieval_time"] = (
                ...
                * (self.system_metrics["total_retrievals"] - 1)
                + retrieval_time
            ) / self.system_metrics["total_retrievals"]
    ...
    def analyze_context_quality(
        self, session_id: str, context_chunks: List[str], query: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_chunk_length = (
                sum(len(chunk) for chunk in context_chunks) / len(context_chunks)
                if context_chunks
                else 0
            )
    ...
    def _generate_quality_recommendations(
        ...
        if metrics.redundancy_score > 0.7:
            recommendations.append(
                "High redundancy detected. Consider improving chunk diversity."
            )

        if metrics.relevance_score < 0.3:
            recommendations.append(
                "Low relevance score. Consider refining search query or improving indexing."
            )

        if metrics.unique_sources < 2:
            recommendations.append(
                "Limited source diversity. Consider expanding knowledge base."
            )

        if metrics.avg_chunk_length < 100:
            recommendations.append(
                "Chunks may be too short. Consider adjusting chunking strategy."
            )

        if metrics.freshness_score < 0.3:
            recommendations.append(
                "Context may be outdated. Consider refreshing knowledge base."
            )
    ...
    def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_retrievals = [
                m for m in self.retrieval_metrics if m.timestamp >= cutoff_time
            ]
            recent_context_quality = [
                m for m in self.context_quality_metrics if m.timestamp >= cutoff_time
            ]
            ...
            summary = {
                ...
                "retrieval_stats": {
                    ...
                    "avg_retrieval_time": (
                        sum(m.retrieval_time for m in recent_retrievals)
                        / len(recent_retrievals)
                        if recent_retrievals
                        else 0
                    ),
                    "avg_nodes_retrieved": (
                        sum(m.retrieved_nodes for m in recent_retrievals)
                        / len(recent_retrievals)
                        if recent_retrievals
                        else 0
                    ),
                    "avg_similarity_score": (
                        sum(m.avg_similarity_score for m in recent_retrievals)
                        / len(recent_retrievals)
                        if recent_retrievals
                        else 0
                    ),
                ...
                "context_quality_stats": {
                    ...
                    "avg_redundancy_score": (
                        sum(m.redundancy_score for m in recent_context_quality)
                        / len(recent_context_quality)
                        if recent_context_quality
                        else 0
                    ),
                    "avg_relevance_score": (
                        sum(m.relevance_score for m in recent_context_quality)
                        / len(recent_context_quality)
                        if recent_context_quality
                        else 0
                    ),
                    "avg_freshness_score": (
                        sum(m.freshness_score for m in recent_context_quality)
                        / len(recent_context_quality)
                        if recent_context_quality
                        else 0
                    ),
    ...
    def get_query_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for metrics in self.retrieval_metrics:
                ...
                first_word = (
                    metrics.query.split()[0].lower()
                    if metrics.query.split()
                    else "unknown"
                )
            ...
            for pattern, queries in query_patterns.items():
                pattern_stats[pattern] = {
                    "count": len(queries),
                    "avg_retrieval_time": sum(q.retrieval_time for q in queries)
                    / len(queries),
                    "avg_nodes_retrieved": sum(q.retrieved_nodes for q in queries)
                    / len(queries),
                    "avg_similarity_score": sum(q.avg_similarity_score for q in queries)
                    / len(queries),
                }
...
def analyze_context():
    ...
    try:
        ...
        result = debug_manager.analyze_context_quality(
            session_id, context_chunks, query
        )
        return success_response(result)
...
def get_system_health():
    ...
    try:
        ...
        if os.path.exists(docstore_path):
            ...
            vector_size = (
                os.path.getsize(vector_store_path)
        ...
        for entry in os.listdir(index_root):
            ...
            if os.path.isdir(entry_path):
                ...
                if os.path.exists(project_docstore):
                    ...
                    project_vector = os.path.join(
                        entry_path, "default__vector_store.json"
                    )
        ...
        recent_errors = len(
            [
                e
                for e in debug_manager.error_log
        ...
        if (
            index_stats["access_stats"]["errors"]
            > index_stats["access_stats"]["total_loads"] * 0.1
        ):
            health_score -= 0.2
        ...
        health_level = (
            ...
            if health_score >= 0.9
                ...
                "good"
                if health_score >= 0.7
                else "fair" if health_score >= 0.5 else "poor"
            )
        )
        ...
        return success_response(
            {
                ...
                "uptime": (
                    datetime.now() - debug_manager.system_metrics["last_reset"]
                ).total_seconds(),
            }
        )
