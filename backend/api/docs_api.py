...
def _is_code_file(filename: str) -> bool:
    ...
    code_extensions = {
        ...
        ".Dockerfile",  # Docker
        ".md",
        ".markdown",  # Markdown (often has code)
        ".vue",  # Vue.js
        ".svelte",  # Svelte
        ".dart",  # Dart
        ".r",
...
def _read_file_content(file_path: str, max_size_mb: int = 100) -> str:
    ...
    try:
        ...
        if file_size > max_size_bytes:
            logger.warning(
                f"File {file_path} is too large ({file_size} bytes, limit: {max_size_bytes} bytes) to store content"
            )
            return None
        ...
        try:
            ...
            for doc in paginated_docs.items:
                ...
                if doc_data.get("indexing_job_id"):
                    try:
                        progress_info = progress_system.get_process(
                            doc_data["indexing_job_id"]
                        )
                        if progress_info:
                            progress_status = (
                                progress_info.status.value
                                if hasattr(progress_info.status, "value")
                                else str(progress_info.status)
                            )
                            doc_status = doc_data.get("index_status")

                            # Fix inconsistencies between progress and document status
                            if (
                                progress_status in ["in_progress", "pending"]
                                and doc_status == "INDEXED"
                            ):
                                logger.warning(
                                    f"Document {doc.id} shows INDEXED but progress is {progress_status}"
                                )
                                doc_data["index_status"] = "INDEXING"
                                doc_data["status_note"] = (
                                    f"Corrected from progress system ({progress_status})"
                                )
                            elif progress_status == "error" and doc_status in [
                                "INDEXED",
                                "INDEXING",
                            ]:
                                logger.warning(
                                    f"Document {doc.id} shows {doc_status} but progress failed"
                                )
                                doc_data["index_status"] = "ERROR"
                                doc_data["status_note"] = (
                                    "Corrected from failed progress"
                                )
                    except Exception as progress_check_error:
                        logger.debug(
                            f"Could not validate progress for document {doc.id}: {progress_check_error}"
                        )
...
def get_document_usage(doc_id):
    ...
    try:
        ...
        if index_manager and hasattr(index_manager, "get_cache_stats"):
            ...
            retrieval_stats["retrieval_count"] = (
                cache_stats.get("cache_hits", 0) // 10
            )  # Rough estimate
            retrieval_stats["avg_similarity_score"] = (
                0.75  # Default score for indexed documents
            )
        ...
        if file_size > 0:
            retrieval_stats["context_hits"] = max(
                1, file_size // 1024
            )  # Rough estimate
...
def get_document_context(doc_id):
    ...
    try:
        ...
        if document.tags:
            try:
                tags = (
                    json.loads(document.tags)
                    if isinstance(document.tags, str)
                    else document.tags
                )
                if isinstance(tags, list):
                    entities = [
                        tag for tag in tags if len(tag) > 2
    ...
    try:
        ...
        if document.content is not None:
            ...
            content_info = {
                ...
                "storage_type": (
                    "complete" if document.index_status == "STORED" else "truncated"
                ),
            }
...
def manage_document(doc_id):
    ...
    if request.method == "GET":
        ...
        logger.info(
            f"API: DELETE /api/docs/{doc_id} - Enhanced deletion for code files"
        )

        # Enhanced deletion: Handle code files vs indexed files differently
        is_code_file = (
            document.is_code_file if hasattr(document, "is_code_file") else False
        )
        index_status = document.index_status
        ...
        if hasattr(document, "indexing_job_id") and document.indexing_job_id:
            try:
                ...
                progress_system = get_unified_progress()
                logger.info(
                    f"Cancelling indexing job {document.indexing_job_id} for document {doc_id}"
                )
                progress_system.cancel_process(
                    document.indexing_job_id, f"Document {doc_id} deleted"
                )
                logger.info(
                    f"Successfully cancelled indexing job {document.indexing_job_id}"
                )
            except Exception as cancel_error:
                logger.warning(
                    f"Failed to cancel indexing job {document.indexing_job_id}: {cancel_error}"
                )
        ...
        if not is_code_file and index_status in ["INDEXED", "INDEXING"]:
            index_instance: Optional[VectorStoreIndex] = current_app.config.get(
                "LLAMA_INDEX_INDEX"
            )
            ...
            if (
                ...
                and storage_dir
                and hasattr(index_instance, "storage_context")
                and hasattr(index_instance, "delete_ref_doc")
            ):

                try:
                    ref_doc_id_to_delete = str(document.id)
                    index_instance.delete_ref_doc(
                        ref_doc_id_to_delete, delete_from_docstore=True
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Persisted index changes after deleting doc {doc_id}"
                        )
                    ...
                    logger.warning(
                        f"Failed to remove from index (continuing with deletion): {index_err}"
                    )
            else:
                logger.info(
                    f"Index not available for document {doc_id} - skipping index removal"
                )
        else:
            logger.info(
                f"Document {doc_id} is a code file (stored={index_status}) - skipping index removal"
            )
...
def upload_document():
    ...
    try:
        ...
        return (
            jsonify({"error": "Database error during upload", "details": str(e)}),
            500,
        )
    except Exception as e:
        logger.error(
            f"API Error (POST /docs/upload): Unexpected error: {e}", exc_info=True
        )
...
def trigger_reindex(doc_id):
    ...
    logger.info(
        f"API: Received POST /api/docs/{doc_id}/reindex request - redirecting to unified indexing"
    )
    ...
    try:
        ...
        with current_app.test_request_context(
            json={"parent_job_id": parent_job_id} if parent_job_id else {}
        ):
        ...
        logger.error(
            f"Failed to trigger unified reindexing for doc {doc_id}: {e}", exc_info=True
        )
