...
enhanced_gen_bp = Blueprint(
    "enhanced_generation_api", __name__, url_prefix="/api/enhanced-generation"
)
    ...
    try:
        ...
        try:
            ...
            if result.get("success"):
                logger.info(
                    f"Enhanced CSV generation completed: {result.get('output_filename')}"
                )
        ...
        try:
            ...
            return success_response(
                ...
                data={
                    ...
                    "entity_relationships_summary": context.entity_relationships.get(
                        "summary", ""
                    ),
                    "competitor_content": {
                        ...
                        "keywords_found": len(
                            context.competitor_content.get("keywords", [])
                        ),
                        "products_found": len(
                            context.competitor_content.get("products", [])
                        ),
                    },
                    "client_documents": [
                        {
                            ...
                            "keywords_extracted": len(
                                doc.get("extracted_keywords", [])
                            ),
                        }
                        for doc in context.client_documents
                    ],
                    "industry_context": (
                        context.industry_context[:200] + "..."
                        if len(context.industry_context) > 200
                        else context.industry_context
                    ),
                    ...
                    "context_completeness": {
                        ...
                        "has_competitor_data": bool(
                            context.competitor_content.get("content")
                        ),
    ...
    try:
        ...
        capabilities = {
            ...
            "generation_readiness": {
                ...
                "recommended_competitor_url": (
                    "https://example.com/competitor" if client.notes else None
                ),
        ...
        return success_response(
            message=f"Client capabilities for {client.name}", data=capabilities
        )
...
def test_enhanced_integration():
    ...
    try:
        ...
        working_components = sum(
            1 for result in test_results.values() if result.get("status") == "working"
        )
        ...
        integration_status = {
            "overall_status": (
                ...
                else "partial" if working_components >= 2 else "not_ready"
            ),
        ...
        return success_response(
            message="Enhanced integration test completed", data=integration_status
        )
