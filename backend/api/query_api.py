...
try:
    ...
    from backend.config import LLM_REQUEST_TIMEOUT as DEFAULT_LLM_REQUEST_TIMEOUT
...
def handle_createcsv_command(args_string: str, session_id: str):
    ...
    if rule_id_val and items_str:
        ...
        if not isinstance(rule_id_val, int):
            return (
                jsonify({"error": "Legacy format: rule_id must be an integer"}),
                400,
            )

        # Route to legacy batch CSV generation
        return _handle_legacy_createcsv(
            rule_id_val, secure_filename_val, items, session_id
        )
        ...
        if (
            not natural_language_instructions
            or len(natural_language_instructions.strip()) < 10
        ):
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"Natural language instructions: {natural_language_instructions[:200]}..."
        )

        try:
            # Call the bulk generation API with natural language processing
            bulk_endpoint_url = url_for(
                "bulk_generation_api.generate_bulk_csv", _external=True
            )
            ...
            logger.info(
                f"Bulk generation payload for natural language /createcsv: {bulk_payload}"
            )
            ...
            if response.status_code in [200, 202]:  # 200 OK or 202 Accepted
                    ...
                    jsonify(
                        {
                            ...
                            "estimated_items": bulk_result.get(
                                "num_items", "auto-detected"
                            ),
                ...
                return (
                    jsonify({"error": f"Failed to start bulk generation: {error_msg}"}),
                    500,
                )
            ...
            logger.error(
                f"Unexpected error in natural language /createcsv: {e}", exc_info=True
            )
            return jsonify({"error": f"Unexpected error: {str(e)}"}), 500


def _handle_legacy_createcsv(
    rule_id_val: int, secure_filename_val: str, items: List[str], session_id: str
):
    """Handle legacy /createcsv format with rule_id and items list."""
    logger.info(
        f"Using LEGACY format for /createcsv with rule_id={rule_id_val} and {len(items)} items"
    )
    ...
    try:
        ...
        if response.status_code in [200, 202]:  # 200 OK or 202 Accepted
            ...
            error_msg = (
                f"Bulk generation API error: {response.status_code} - {response.text}"
            )
            logger.error(error_msg)
            return (
                jsonify({"error": f"Failed to convert legacy format: {error_msg}"}),
                500,
            )
        ...
        logger.error(
            f"Unexpected error in legacy /createcsv conversion: {e}", exc_info=True
        )
        return (
            jsonify({"error": f"Unexpected error in legacy conversion: {str(e)}"}),
            500,
        )


def _should_use_bulk_generation(
    args_string: str, raw_user_specifications: str, items_count: int = 0
) -> tuple[bool, dict]:
    """
    ...
    if (
        "professional services" in content_combined
        or "data center legal" in content_combined
    ):
        should_use_bulk = True
    ...
    if any(
        word in content_combined
        for word in ["wordpress", "seo", "landing pages", "website content"]
    ):
    ...
    bulk_params = {
        "num_items": (
            max(detected_quantity, 10) if should_use_bulk else detected_quantity
        ),
        "target_word_count": detected_word_count,
        "concurrent_workers": (
            min(15, max(5, detected_quantity // 10)) if detected_quantity > 0 else 10
        ),
        "client": (
            "Professional Services"
            if "professional services" in content_combined
            else "Professional Services"
        ),
        "website": (
            "datacenterknowledge.com/business"
            if "data center" in content_combined
            else "professional-website.com"
        ),
...
def handle_generatecsv_command(args_string: str, session_id: str):
    ...
    filename = (
        parsed_kwargs.get("filename")
        or parsed_kwargs.get("output_file")
        or "generated_content.csv"
    )
    ...
    if not secure_filename_val:
        return (
            jsonify({"error": f"Invalid filename provided: '{filename}'"}),
            400,
        )

    # SMART ROUTING: Check if this should use bulk generation
    should_use_bulk, bulk_params = _should_use_bulk_generation(
        args_string, raw_user_specifications
    )
    ...
    if should_use_bulk:
        logger.info(
            f"Routing to BULK generation API. Reasons: {bulk_params['reasons']}"
        )
        ...
        try:
            ...
            bulk_payload = {
                ...
                "batch_size": min(
                    25, bulk_params["num_items"] // 4
                ),  # Smaller batches for better resumability
                "resume_from_id": None,
            }
 ...
 **Progress Tracking**: Monitor progress via the Tasks page or check logs for real-time updates."""
            ...
            logger.error(
                f"Error calling bulk CSV generation service: {e}", exc_info=True
            )
            error_detail = str(e)
    ...
    try:
        ...
        generation_payload = {
            ...
            "output_filename": secure_filename_val,
            "args_string": raw_user_specifications
            or f"Generate CSV content: {filename}",
        }
        ...
        if response.status_code == 200:
            response_data = response.json()
            logger.info(
                f"Single CSV generation successful: {response_data.get('output_file', 'unknown')}"
            )
            ...
            logger.error(
                f"Single CSV generation failed: {response.status_code} - {response.text}"
            )
...
def handle_generatefile_command(args_string: str, session_id: str):
    ...
    # Extract filename from command or use default
    output_file = (
        parsed_kwargs.get("output_file")
        or parsed_kwargs.get("filename")
        or "generated_content.txt"
    )

    secure_output_filename = secure_filename(output_file)
    if not secure_output_filename:
        return (
            jsonify({"error": f"Invalid output_file name provided: '{output_file}'"}),
            400,
        )
...
def handle_batchcsv_command(args_string: str, session_id: str):
    ...
    if any(
        keyword in content_combined
        for keyword in ["data center", "datacenter", "imperial"]
    ):
        ...
        keyword in content_combined
        for keyword in ["tech", "software", "cloud", "computing"]
    ):
...
def _handle_rag_query(user_prompt: str, session_id: str):
    ...
    try:
        ...
        if entity_nodes:
            ...
            for node in entity_nodes[:5]:  # Limit to top 5 entity results
                entity_context_parts.append(
                    f"[{node.metadata.get('entity_type', 'Entity').upper()}] {node.text}"
                )
        ...
        logger.info(
            f"Retrieved context - Entities: {len(entity_nodes)}, Documents: {len(document_nodes)}, CSV Summaries: {len(csv_summary_nodes)}"
        )
...
def query_endpoint():
    ...
    try:
        ...
        if bypass_rules:
            logger.info(
                "Rules bypass enabled - using minimal template for code intelligence"
            )
            qa_template_string = "{context_str}\n\n{query_str}"
