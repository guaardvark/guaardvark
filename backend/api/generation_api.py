...
def _validate_and_enhance_jsx_content(content: str, filename: str) -> str:
    ...
    has_react_import = bool(
        re.search(r'import\s+(?:React|.*?)\s+from\s+[\'"]react[\'"]', content)
    )

    # Check if there's a component definition
    has_component = bool(
        re.search(r"(?:function|const|class)\s+([A-Z][a-zA-Z0-9_]*)", content)
    )
...
def _validate_and_enhance_python_content(content: str, filename: str) -> str:
    ...
    has_docstring = bool(re.search(r'""".*?"""', content, re.DOTALL)) or bool(
    ...
    if (
        "List[" in enhanced_content
        or "Dict[" in enhanced_content
        or "Optional[" in enhanced_content
    ):
        if (
            "from typing import" not in enhanced_content
            and "import typing" not in enhanced_content
        ):
    ...
    if "datetime." in enhanced_content or "timedelta" in enhanced_content:
        if (
            "import datetime" not in enhanced_content
            and "from datetime import" not in enhanced_content
        ):
    ...
    if common_imports_needed:
        ...
        if enhanced_content.startswith('"""'):
            ...
            enhanced_content = (
                enhanced_content[:docstring_end]
                + "\n\n"
                + imports_section
                + enhanced_content[docstring_end:].lstrip()
            )
        else:
    ...
    if not has_main_block:
        ...
        if has_module_level_calls:
            logger.info(
                f"Adding if __name__ == '__main__' block to Python file: {filename}"
            )
    ...
    enhanced_content = re.sub(
        r"\bprint\s+([^(].*?)$", r"print(\1)", enhanced_content, flags=re.MULTILINE
    )
...
def direct_generate_and_save_file_route():
    ...
    with coordinator.managed_operation(
        ...
        try:
            ...
            try:
                secure_filename_result = secure_file_manager.secure_filename(
                    output_filename
                )
                if not secure_filename_result or secure_filename_result.strip() == "":
                    raise ValueError(
                        "Filename resulted in empty string after sanitization"
                    )
            ...
            if not coordinator.validate_security(
                "file_operation", file_path=output_path, operation="write"
            ):
                return (
                    jsonify(
                        {
            ...
            try:
                ...
                if rule_id:
                    ...
                    codegen_rule = (
                        ...
                        .filter_by(id=rule_id, is_active=True)
                        .first()
                    )
                    if codegen_rule:
                        logger.info(
                            f"Frontend rule found: ID={codegen_rule.id}, name={codegen_rule.name}"
                        )
                    else:
                        logger.warning(
                            f"Frontend-provided rule ID {rule_id} not found or inactive"
                        )

                if not codegen_rule:
                    # Fallback to default CodeGen rule
                    logger.info(
                        f"Attempting to retrieve default CodeGen rule for model: {model_name}"
                    )
                    codegen_rule = rule_utils.get_active_command_rule(
                        "/codegen", db.session, model_name=model_name
                    )
                    logger.info(
                        f"Default CodeGen rule retrieval result: {codegen_rule is not None}"
                    )

                if codegen_rule:
                    logger.info(
                        f"Using rule: ID={codegen_rule.id}, length={len(codegen_rule.rule_text)}"
                    )
                ...
                if system_prompt:
                    logger.info(
                        f"Using RulesPage system prompt for file generation (rule ID: {final_rule_id})"
                    )
...
Generate the complete file content now:"""
                ...
                logger.warning(
                    f"Failed to get RulesPage prompt for file generation: {e}"
                )
...
Generate the complete file content now:"""
            ...
            try:
                ...
                from backend.services.entity_relationship_indexer import (
                    EntityRelationshipIndexer,
                )

                logger.info(
                    f"RAG GENERATION: Using proper RAG search for context retrieval"
                )
                ...
                try:
                    ...
                    if rag_results is None:
                        rag_results = []
                    logger.info(
                        f"RAG GENERATION: Retrieved {len(rag_results)} chunks from vector search"
                    )
                ...
                if project_id:
                    ...
                    logger.info(
                        f"RAG GENERATION: Retrieved {len(recent_files)} project files for project {project_id}"
                    )
                else:
                    recent_files = (
                        db.session.query(DBDocument)
                    ...
                    logger.info(
                        f"RAG GENERATION: Retrieved {len(recent_files)} recent files (no project_id)"
                    )
                ...
                if rag_results or recent_files:
                    ...
                    if rag_results and len(rag_results) > 0:
                        ...
                        logger.info(
                            f"RAG search returned no relevant results for query: {prompt_text[:100]} - this is normal for new content generation"
                        )
                        context_parts.append(
                            "VECTOR SEARCH: No relevant indexed content found for this query. Proceeding with standalone generation."
                        )
                    ...
                    uploaded_file_context = "\n".join(context_parts)
                    logger.info(
                        f"RAG GENERATION: Built context with {len(rag_results)} RAG chunks + {len(recent_files)} project files"
                    )
            ...
            try:
                ...
                if uploaded_file_context:
                    enhanced_prompt = (
                        f"{uploaded_file_context}\n\nUSER REQUEST: {prompt_text}"
                    )
                    logger.info(
                        f"ENHANCED GENERATION: Using file context ({len(uploaded_file_context)} chars)"
                    )
                    logger.info(
                        f"ENHANCED GENERATION: Enhanced prompt length: {len(enhanced_prompt)} chars"
                    )
                else:
                ...
                logger.info(
                    f"DEBUG GENERATION: System message preview: {system_message[:200]}..."
                )
                logger.info(
                    f"DEBUG GENERATION: System message length: {len(system_message)} chars"
                )
                logger.info(
                    f"DEBUG GENERATION: Enhanced prompt preview: {enhanced_prompt[:200]}..."
                )
                logger.info(
                    f"DEBUG GENERATION: Enhanced prompt length: {len(enhanced_prompt)} chars"
                )
                ...
                generated_content = llm_service.run_llm_chat_prompt(
                    ...
                    messages=[
                        llm_service.ChatMessage(
                            role=llm_service.MessageRole.SYSTEM, content=system_message
                        ),
                        llm_service.ChatMessage(
                            role=llm_service.MessageRole.USER, content=enhanced_prompt
                        ),
                    ],
                )
                ...
                logger.info(
                    f"DEBUG GENERATION: LLM response preview: {str(generated_content)[:300]}..."
                )
                logger.info(
                    f"DEBUG GENERATION: LLM response length: {len(str(generated_content))} chars"
                )

                if not generated_content or not str(generated_content).strip():
                    logger.warning(
                        f"LLM returned empty content for prompt: {enhanced_prompt[:100]}..."
                    )
                    return (
                        jsonify(
                            {
                        ...
                        422,
                    )  # Unprocessable Entity instead of Internal Server Error
                ...
                if output_filename:
                    ...
                    if filename_lower.endswith(".jsx"):
                        generated_content = _validate_and_enhance_jsx_content(
                            generated_content, output_filename
                        )
                    elif filename_lower.endswith(".py"):
                        generated_content = _validate_and_enhance_python_content(
                            generated_content, output_filename
                        )
            ...
            try:
                ...
                if not os.path.exists(full_path):
                    raise FileNotFoundError(
                        f"Generated file was not created: {full_path}"
                    )
                ...
                logger.info(
                    f"Successfully generated file: {full_path} ({file_size} bytes)"
                )
            ...
            logger.error(
                f"Unexpected error in direct_generate_and_save: {e}", exc_info=True
            )
            return (
                jsonify(
                    {
...
def generate_from_command():
    ...
    with coordinator.managed_operation(
        "command_generation", ProcessType.FILE_GENERATION
    ) as process_id:
        ...
        while retry_count < max_retries and generated_content is None:
            try:
                ...
                is_csv_file = (
                    "csv" in command_label.lower()
                    or "csv" in final_prompt.lower()
                    or output_filename.lower().endswith(".csv")
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Detected target format: {target_format} for file: {output_filename}"
                    )
                except ImportError:
                    target_format = None
                ...
                if is_csv_file:
                    ...
                    try:
                        from backend.utils.csv_formatter import (
                            generate_csv_prompt,
                            CSVFormatter,
                        )
                        ...
                        try:
                            template = csv_formatter.detect_csv_template(
                                "", final_prompt
                            )
                            enhanced_prompt = (
                                csv_formatter.generate_structured_csv_prompt(
                        ...
                        logger.info(
                            f"Using enhanced CSV prompt generation for file: {output_filename}"
                        )

                    except ImportError as e:
                        logger.warning(
                            f"CSV formatter not available, using fallback prompt: {e}"
                        )
...
Generate the complete file content now:"""
                ...
                generated_content = llm_service.run_llm_chat_prompt(
                    ...
                    messages=[
                        ...
                        llm_service.ChatMessage(
                            role=llm_service.MessageRole.USER, content=final_prompt
                        ),
                    ],
                )
                ...
                if not generated_content or not generated_content.strip():
                    logger.warning(
                        f"LLM returned empty content on attempt {retry_count}"
                    )
                    generated_content = None
                    continue
                ...
                if target_format and file_processor:
                    try:
                        ...
                        if target_format == FileFormat.CSV or is_csv_file:
                            ...
                            formatted_content = format_csv_content(
                                generated_content, final_prompt
                            )

                            if formatted_content != generated_content:
                                logger.info(
                                    f"Applied CSV formatting - length changed from {original_length} to {len(formatted_content)} characters"
                                )
                            ...
                            if not generated_content.strip().startswith("<?xml"):
                                ...
                                logger.info(
                                    "Added XML declaration to generated content"
                                )
                            ...
                            try:
                                ...
                                logger.warning(
                                    "Generated content is not valid JSON, wrapping in basic structure"
                                )
                                generated_content = json.dumps(
                            ...
                            if not generated_content.strip().startswith(
                                "<!DOCTYPE"
                            ) and not generated_content.strip().startswith("<html"):
                                generated_content = f"<!DOCTYPE html>\n<html>\n<head>\n<title>Generated Content</title>\n</head>\n<body>\n{generated_content}\n</body>\n</html>"
                                logger.info(
                                    "Added HTML document structure to generated content"
                                )

                        logger.info(
                            f"Applied {target_format.value.upper()} formatting to generated content"
                        )

                    except ImportError as e:
                        logger.warning(
                            f"Enhanced file processor not available for post-processing: {e}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Format-specific processing failed, using original content: {e}"
                        )
                    ...
                    try:
                        ...
                        try:
                            ...
                            if (
                                formatted_content
                                and formatted_content != generated_content
                            ):
                                logger.info(
                                    f"Applied fallback CSV formatting - length changed from {original_length} to {len(formatted_content)} characters"
                                )
                                generated_content = formatted_content
                            else:
                                logger.info(
                                    "Content was already in proper CSV format or no changes needed"
                                )
                        except (AttributeError, TypeError) as format_error:
                            logger.warning(
                                f"CSV format_csv_content method failed: {format_error}"
                            )
                            # Continue with original content

                    except ImportError as e:
                        logger.warning(
                            f"CSV formatter not available for post-processing: {e}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"CSV formatting failed, using original content: {e}"
                        )
        ...
        try:
            # SECURITY FIX: Secure filename validation
            secure_file_manager = get_secure_file_manager()
            secure_output_filename = secure_file_manager.secure_filename(
                output_filename
            )
            ...
            if not coordinator.validate_security(
                "file_operation", file_path=full_path, operation="write"
            ):
                return (
                    jsonify(
                        {
            ...
            logger.info(
                f"Successfully wrote file from command '{command_label}': {final_path} ({file_size} bytes)"
            )
            ...
            logger.error(
                f"Unexpected error writing file '{output_filename}': {e}", exc_info=True
            )
            return (
                jsonify(
                    {
