...
TRACKING_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "data", "outputs", "tracking"
)
    ...
    for filename in os.listdir(TRACKING_DIR):
        if filename.endswith(".json") and "tracking" in filename:
            ...
            try:
                ...
                replaced_rows = len(
                    [r for r in row_records if r.get("replacement_count", 0) > 0]
                )
                ...
                files.append(
                    {
                        ...
                        "success_rate": (
                            (active_rows / total_rows * 100) if total_rows > 0 else 0
                        ),
...
def list_outputs():
    ...
    try:
        ...
        return jsonify({"success": True, "outputs": files, "total": len(files)})
    except Exception as e:
...
def download_csv(filename):
    ...
    try:
        ...
        return send_file(
            csv_path, as_attachment=True, download_name=base_name, mimetype="text/csv"
        )
    except Exception as e:
    ...
    try:
        ...
        if original_job_id:
            ...
            for retry_filename in os.listdir(TRACKING_DIR):
                if (
                    retry_filename.startswith("retry_")
                    and retry_filename.endswith(".json")
                    and retry_filename != filename
                ):
                    ...
                    try:
                        ...
                        if retry_metadata.get(
                            "original_job_id"
                        ) == original_job_id or retry_metadata.get(
                            "job_id", ""
                        ).startswith(
                            "retry_"
                        ):
                        ...
                        logger.warning(
                            f"Error reading retry file {retry_filename}: {e}"
                        )
                        continue
