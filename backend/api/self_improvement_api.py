...
self_improvement_bp = Blueprint(
    "self_improvement", __name__, url_prefix="/api/self-improvement"
)
...
def get_status():
    ...
    last_run = (
        ...
        .order_by(SelfImprovementRun.timestamp.desc())
        .first()
    )

    total_fixes = (
        db.session.query(SelfImprovementRun).filter_by(status="success").count()
    )

    return success_response(
        data={
            "enabled": (
                enabled_setting.value.lower() == "true" if enabled_setting else False
            ),
            "codebase_locked": (
                (locked_setting and locked_setting.value.lower() == "true")
...
def toggle_self_improvement():
    ...
    if setting:
        ...
        db.session.add(
            SystemSetting(key="self_improvement_enabled", value=str(enabled).lower())
        )
    db.session.commit()
