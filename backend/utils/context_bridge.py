...
class ContextBridge:
    ...
    def _on_client_created(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Client {target.id} ({target.name}) created - scheduling metadata indexing"
            )
    ...
    def _on_client_updated(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Client {target.id} ({target.name}) updated - scheduling metadata reindexing"
            )
            self._schedule_indexing("client", target.id)
        except Exception as e:
    ...
    def _on_client_deleted(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Client {target.id} ({target.name}) deleted - metadata will be removed from index"
            )
    ...
    def _on_project_created(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Project {target.id} ({target.name}) created - scheduling metadata indexing"
            )
    ...
    def _on_project_updated(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Project {target.id} ({target.name}) updated - scheduling metadata reindexing"
            )
            self._schedule_indexing("project", target.id)
    ...
    def _on_task_created(self, mapper, connection, target):
        ...
        try:
            logger.info(
                f"Context Bridge: Task {target.id} ({target.name}) created - scheduling metadata indexing"
            )
            ...
            if target.project_id:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not get project {target.project_id} for client indexing: {db_error}"
                    )
    ...
    def _on_task_updated(self, mapper, connection, target):
        ...
        try:
            ...
            if any(attr in changed_attrs for attr in significant_changes):
                logger.info(
                    f"Context Bridge: Task {target.id} ({target.name}) updated significantly - scheduling metadata reindexing"
                )
                self._schedule_indexing("job", target.id)
    ...
    def _on_document_updated(self, mapper, connection, target):
        ...
        try:
            ...
            if "index_status" in changed_attrs and target.project_id:
                logger.info(
                    f"Context Bridge: Document {target.id} indexing status changed - updating project context"
                )
                self._schedule_indexing("project", target.project_id)
    ...
    def _schedule_indexing(self, entity_type: str, entity_id: int):
        ...
        try:
            ...
            if (
                db
                and db.session
                and hasattr(db.session, "in_transaction")
                and db.session.in_transaction()
            ):
                # We're in an active transaction - defer the indexing to avoid conflicts
                logger.debug(
                    f"Deferring metadata indexing for {entity_type} {entity_id} due to active transaction"
                )
            ...
            logger.debug(
                f"Error scheduling indexing for {entity_type} {entity_id}: {e}"
            )
    ...
    def _execute_indexing(self, entity_type: str, entity_id: int):
        ...
        try:
            ...
            logger.warning(
                f"Indexing service not available, skipping {entity_type} metadata indexing"
            )
    ...
    def manual_sync_context(self, entity_type: str, entity_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error in manual context sync for {entity_type} {entity_id}: {e}"
            )
            return False
    ...
    def get_context_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "metadata_indexing_available": metadata_indexing_service.add_text_to_index
                is not None,
            ...
            return {"bridge_enabled": False, "error": str(e)}
