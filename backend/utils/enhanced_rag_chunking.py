...
class BaseChunker(ABC):
    ...
    def _detect_content_type(self, text: str) -> str:
        ...
        # List patterns
        if re.match(r"^\s*[-*+]\s", text, re.MULTILINE) or re.match(
    ...
    def _detect_language(self, text: str) -> str:
        ...
        if any(char in text for char in ["ñ", "ü", "é", "à", "ç"]):
            ...
            if "que " in text_lower or "para " in text_lower or "con " in text_lower:
                ...
                "pour " in text_lower or "avec " in text_lower or "dans " in text_lower
            ):
                return "fr"  # French
            else:
        ...
        if any(
            ...
            for pattern in ["function", "class", "import", "def ", "const ", "var "]
        ):
            return "code"
...
class HierarchicalChunker(BaseChunker):
    ...
    def __init__(self, strategy: ChunkingStrategy):
        ...
        self.node_parser = HierarchicalNodeParser.from_defaults(
            chunk_sizes=[
                ...
                strategy.max_chunk_size // 2,
                strategy.max_chunk_size // 4,
            ],
        ...
        try:
            ...
            for i, node in enumerate(nodes):
                enhanced_metadata = self._create_enhanced_metadata(
                    node.get_content(), document.doc_id or f"doc_{i}", i
                )
    ...
    def _create_enhanced_metadata(
        self, content: str, source_id: str, position: int
    ) -> Dict[str, Any]:
        ...
        entities = (
            self._extract_entities(content) if self.strategy.extract_entities else []
        )
        ...
        if CACHE_AVAILABLE:
            set_cached_metadata(
                source_id, content_hash, metadata, ttl=86400
            )  # 24 hours
    ...
    def _simple_chunk_document(self, document: LlamaDocument) -> List[BaseNode]:
        ...
        for i, chunk in enumerate(chunks):
            metadata = self._create_enhanced_metadata(
                chunk, document.doc_id or f"doc_{i}", i
            )
            ...
            node = TextNode(text=chunk, metadata=metadata, id_=metadata["chunk_id"])
            nodes.append(node)
...
class SemanticChunker(BaseChunker):
    ...
    def __init__(self, strategy: ChunkingStrategy):
        ...
        try:
            ...
            # ==========================================================================
            local_embed_model = (
                backend.utils.llama_index_local_config.get_local_embedding_model()
            )
            ...
            if local_embed_model:
                ...
                logger.info(
                    "SemanticSplitterNodeParser configured with local embeddings"
                )
            else:
            ...
            logger.error(
                f"Unexpected error configuring SemanticSplitterNodeParser: {e}"
            )
            self.node_parser = None
...
class AdaptiveChunker(BaseChunker):
    ...
    def _analyze_content_for_strategy(self, content: str) -> str:
        ...
        code_score = sum(
            1 for indicator in code_indicators if indicator in content.lower()
        )
...
class CodeChunker(BaseChunker):
    ...
    def _chunk_code_file(
        self, document: LlamaDocument, language: str
    ) -> List[BaseNode]:
        ...
        if file_size <= 50000:
            ...
            node = TextNode(
                text=content, id_=chunk_metadata.chunk_id, metadata=enhanced_metadata
            )
            nodes.append(node)
        ...
        for i, section in enumerate(sections):
            ...
            chunk_metadata = ChunkMetadata(
                chunk_id=self._generate_chunk_id(
                    section, f"{document.doc_id}_section_{i}"
                ),
                source_document=document.doc_id,
            ...
            node = TextNode(
                text=section, id_=chunk_metadata.chunk_id, metadata=enhanced_metadata
            )
            nodes.append(node)
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith("class "):
                ...
                current_section.extend(
                    imports_section
                )  # Include imports in each section
                current_section.append(line)
                in_class = True
    ...
    def _split_markup_sections(self, content: str) -> List[str]:
        ...
        for match in re.finditer(
            r"<(html|head|body|main|section|article)\b[^>]*>", content, re.IGNORECASE
        ):
    ...
    def _extract_jsx_metadata(self, content: str) -> dict:
        ...
        metadata["components"] = (
            list(set(component_matches)) if component_matches else []
        )
        ...
        metadata["has_react_import"] = bool(
            re.search(self.jsx_patterns["import_react"], content)
        )

        # Check for exports
        metadata["has_export"] = bool(
            re.search(self.jsx_patterns["export_component"], content)
        )
    ...
    def _extract_python_metadata(self, content: str) -> dict:
        ...
        class_matches = re.findall(
            self.python_patterns["class_def"], content, re.MULTILINE
        )
        metadata["classes"] = list(set(class_matches)) if class_matches else []

        # Extract functions
        function_matches = re.findall(
            self.python_patterns["function_def"], content, re.MULTILINE
        )
        metadata["functions"] = list(set(function_matches)) if function_matches else []

        # Extract methods (indented def statements)
        method_matches = re.findall(
            self.python_patterns["method_def"], content, re.MULTILINE
        )
        metadata["methods"] = list(set(method_matches)) if method_matches else []

        # Extract imports
        import_matches = re.findall(
            self.python_patterns["import_statement"], content, re.MULTILINE
        )
        metadata["imports"] = list(set(import_matches)) if import_matches else []

        # Extract decorators
        decorator_matches = re.findall(
            self.python_patterns["decorator"], content, re.MULTILINE
        )
        metadata["decorators"] = (
            list(set(decorator_matches)) if decorator_matches else []
        )

        # Check for docstrings
        metadata["has_docstring"] = bool(
            re.search(self.python_patterns["docstring"], content, re.DOTALL)
        )

        # Check for main block
        metadata["has_main_block"] = bool(
            re.search(self.python_patterns["main_block"], content)
        )

        # Check for exception definitions
        exception_matches = re.findall(self.python_patterns["exception_def"], content)
        metadata["exceptions"] = (
            list(set(exception_matches)) if exception_matches else []
        )

        # Check for constants
        constant_matches = re.findall(
            self.python_patterns["constant"], content, re.MULTILINE
        )
        metadata["constants"] = list(set(constant_matches)) if constant_matches else []

        # Check for type hints
        metadata["has_type_hints"] = bool(
            re.search(self.python_patterns["type_hint"], content)
        )
    ...
    def _chunk_large_python_file(
        ...
        for i, section in enumerate(sections):
            if section.strip():
                ...
                chunk_metadata = ChunkMetadata(
                    chunk_id=self._generate_chunk_id(
                        section, f"{document.doc_id}_python_section_{i}"
                    ),
                    source_document=document.doc_id,
...
class EnhancedRAGChunker:
    ...
    def _detect_best_chunking_strategy(self, document: LlamaDocument) -> str:
        ...
        if not file_extension:
            ...
            if content:
                ...
                code_indicator_count = sum(
                    1 for pattern in code_patterns if pattern in content_lower
                )
                if code_indicator_count >= 3:  # Threshold for code detection
                    logger.info(
                        f"Content-based code detection: {code_indicator_count} patterns found"
                    )
                    return "code"
    ...
    def chunk_documents(
        self, documents: List[LlamaDocument], strategy_name: str = "auto"
    ) -> List[BaseNode]:
        ...
        for document in documents:
            try:
                ...
                if actual_strategy not in self.chunkers:
                    logger.warning(
                        f"Unknown strategy '{actual_strategy}', using adaptive"
                    )
                ...
                logger.info(
                    f"Chunked document {document.doc_id} using '{actual_strategy}' strategy: {len(nodes)} chunks"
                )
    ...
    def get_chunking_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "average_chunks_per_document": (
                ...
                / self.chunking_stats["total_documents"]
                if self.chunking_stats["total_documents"] > 0
                else 0
            ),
        }
    ...
    def add_custom_strategy(
        self, name: str, strategy: ChunkingStrategy, chunker_class: type
    ) -> None:
    ...
    def optimize_strategy_for_content(self, content_sample: str) -> str:
        ...
        has_code = any(
            indicator in content_sample.lower()
        ...
        if has_headers and length > 5000:
            ...
            length > 1000 and (line_count / length * 1000) < 10
        ):  # Dense text with few line breaks
            recommended_strategy = "semantic"

        # Validate that the recommended strategy exists
        if recommended_strategy not in self.chunkers:
            logger.warning(
                f"Recommended strategy '{recommended_strategy}' not available, using adaptive"
            )
