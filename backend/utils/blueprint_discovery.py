...
class BlueprintDiscovery:
    ...
    def discover_blueprints_in_directory(
        self, directory_path: str, package_prefix: str = ""
    ) -> List[Dict[str, Any]]:
        """
        ...
        for py_file in directory.glob("*.py"):
            ...
            try:
                ...
                module_blueprints = self.find_blueprints_in_module(
        ...
        for subdir in directory.iterdir():
            ...
            sub_blueprints = self.discover_blueprints_in_directory(
                str(subdir), sub_package_prefix
            )
            blueprints.extend(sub_blueprints)

        return blueprints

    def find_blueprints_in_module(
        ...
        for attr_name in dir(module):
            ...
            try:
                ...
                if isinstance(attr_value, Blueprint):
                    ...
                    logger.info(
                        f"✓ Discovered blueprint: {attr_name} ({attr_value.name}) in {module_name} with prefix {blueprint_info['url_prefix']}"
                    )
    ...
    def register_blueprints(
        self, app: Flask, blueprints: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for blueprint_info in blueprints:
            ...
            try:
                ...
                if blueprint_name in app.blueprints:
                    logger.debug(
                        f"Skipping already registered blueprint: {blueprint_name}"
                    )
                    skipped_count += 1
                    continue
                ...
                logger.info(
                    f"Registered blueprint: {blueprint_name} from {blueprint_info['module_name']}"
                )
                registered_count += 1
    ...
    def auto_discover_and_register(
        self, app: Flask, directories: List[Tuple[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if directories is None:
            ...
            logger.debug(
                f"API directory: {api_dir} (exists: {os.path.exists(api_dir)})"
            )
            logger.debug(
                f"Routes directory: {routes_dir} (exists: {os.path.exists(routes_dir)})"
            )

            directories = [(api_dir, "backend.api"), (routes_dir, "backend.routes")]
        ...
        for directory_path, package_prefix in directories:
            ...
            dir_blueprints = self.discover_blueprints_in_directory(
    ...
    def log_registration_summary(self, summary: Dict[str, Any]) -> None:
        ...
        if summary["blueprint_details"]:
            ...
            for blueprint in summary["blueprint_details"]:
                ...
                logger.info(
                    f"  - {blueprint['name']} ({prefix}) from {blueprint['module']}"
                )
...
def register_api_blueprints(app: Flask) -> Dict[str, Any]:
    ...
    return discovery.auto_discover_and_register(
        app, directories=[("backend/api", "backend.api")]
    )
...
def register_route_blueprints(app: Flask) -> Dict[str, Any]:
    ...
    discovery = BlueprintDiscovery(app)
    return discovery.auto_discover_and_register(
        app, directories=[("backend/routes", "backend.routes")]
    )
