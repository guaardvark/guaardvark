...
class ContextVariablesExtractor:
            ...
            for pattern in patterns:
                ...
                if matches:
                    if variable_type == "quantity":
                        extracted[variable_type] = (
                            int(matches[0]) if matches[0].isdigit() else None
                        )
                    elif variable_type == "word_count":
                        if isinstance(matches[0], tuple):
                            # Range like "500-1000 words"
                            extracted[variable_type] = (
                                int(matches[0][1]) if matches[0][1].isdigit() else 500
                            )
                        else:
                            extracted[variable_type] = (
                                int(matches[0]) if matches[0].isdigit() else 500
                            )
                    else:
    ...
    def _clean_client_name(self, name: str) -> str:
        ...
        name = re.sub(
            r"\s+(company|corp|corporation|inc|incorporated|llc|ltd|limited)\.?$",
            "",
            name,
            flags=re.IGNORECASE,
        )
...
class ContextVariablesManager:
    ...
    def create_context_from_text(
        self, text: str, template_override: Optional[str] = None
    ) -> ContextTemplate:
    ...
    def get_bulk_generation_payload(
        self, context: ContextTemplate, **overrides
    ) -> Dict[str, Any]:
