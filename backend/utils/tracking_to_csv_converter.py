    ...
    def convert_tracking_to_csv(
        ...
        try:
            ...
            stats = {
                ...
                "file_size": (
                    os.path.getsize(output_csv_path)
            ...
            self.logger.info(
                f"CSV conversion complete: {stats['active_rows_exported']} rows exported"
            )
    ...
    def _write_csv_file(self, active_rows: List[Dict], output_path: str):
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for row_data in active_rows:
                ...
                csv_row["Featured_Image"] = row_data.get(
                    "Featured_Image", row_data.get("Featured Image", "")
                )
                csv_row["Meta_Description"] = row_data.get(
...
def convert_tracking_file(
    tracking_json_path: str, output_csv_path: str, include_inactive: bool = False
) -> Dict:
    """
    ...
    return converter.convert_tracking_to_csv(
        tracking_json_path, output_csv_path, include_inactive
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python tracking_to_csv_converter.py <tracking_json> <output_csv> [include_inactive]"
        )
        sys.exit(1)
