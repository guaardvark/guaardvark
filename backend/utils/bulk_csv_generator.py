...
try:
    ...
    logger.warning(
        "Web search functionality not available - bulk generation will be limited to LLM-only content"
    )
...
class RowTracker:
    ...
    def export_tracking_log_json(
        ...
        if existing_data and "metadata" in existing_data:
            ...
            if (
                status == "in_progress"
                and "export_timestamp" in existing_data["metadata"]
            ):
                tracking_data["metadata"]["export_timestamp"] = existing_data[
                    "metadata"
                ]["export_timestamp"]
            ...
            if "job_parameters" in existing_data["metadata"] and not job_params:
                tracking_data["metadata"]["job_parameters"] = existing_data["metadata"][
                    "job_parameters"
                ]
...
class BulkCSVGenerator:
    ...
    def __init__(
        ...
        logger.info(
            f"Initializing BulkCSVGenerator with output_dir={output_dir}, workers={concurrent_workers}, model={model_name}"
        )
        ...
        self.html_renderer = (
            None  # Will be: HTMLRenderer(...) if structured_html else None
        )

        # FIXED: Circuit breaker to prevent resource waste
        self.consecutive_failures = 0
        self.max_consecutive_failures = (
            8  # Increased threshold - Stop after 8 consecutive failures
        )
        ...
        self._log_info(
            "Attempting to initialize LLM instance", {"requested_model": model_name}
        )
        try:
            if model_name:
            ...
            self._log_info(
                "LLM instance initialized successfully",
                {
            ...
            self._log_error(
                "Failed to initialize LLM instance", {"error": str(e)}, exc_info=True
            )
            self.llm = None
    ...
    def _populate_missing_fields(
        self, content_row: ContentRow, task: GenerationTask
    ) -> ContentRow:
        ...
        if not content_row.category or (
            isinstance(content_row.category, str) and content_row.category.strip() == ""
        ):
            content_row.category = self._generate_category(
                content_row.title, task.topic
            )

        # Generate tags if missing
        if not content_row.tags or (
            isinstance(content_row.tags, str) and content_row.tags.strip() == ""
        ):
            content_row.tags = self._generate_default_tags(
                content_row.category, task.topic
            )

        # Generate slug if missing using enhanced slugify
        if not content_row.slug or (
            isinstance(content_row.slug, str) and content_row.slug.strip() == ""
        ):
            content_row.slug = slugify(
                content_row.title
            )  # Uses enhanced slugify without length limits

        # Set image same as category
        if not content_row.image or (
            isinstance(content_row.image, str) and content_row.image.strip() == ""
        ):
        ...
        if self.structured_html and self.html_renderer:
            try:
                ...
                site_meta = {
                    ...
                    or self.site_meta.get("company_name"),
                    "contact_url": task.contact_url
                    or self.site_meta.get("contact_url"),
                    "primary_service": task.primary_service
                    or self.site_meta.get("primary_service"),
                    "client_location": task.client_location
                    or self.site_meta.get("client_location"),
                    ...
                    "social_links": task.social_links
                    or self.site_meta.get("social_links", []),
                }

                # Convert content to structured HTML
                structured_content = self.html_renderer.render(
                    content_row.content, site_meta
                )
                content_row.content = structured_content

                self._log_info(
                    f"Applied structured HTML to content for task {task.item_id}"
                )

            except Exception as e:
                self._log_error(
                    f"Failed to apply structured HTML for task {task.item_id}: {e}"
                )
    ...
    def _detect_error_patterns(self, response_text: str) -> Optional[tuple]:
        ...
        error_patterns = [
            ...
            (
                "failed_prefix",
                "Failed to generate",
                "Response contains 'Failed to generate'",
            ),
            ...
            (
                "service_unavailable",
                "service unavailable",
                "Response mentions service unavailable",
            ),
            (
                "authentication_error",
                "authentication failed",
                "Response mentions authentication error",
            ),
    ...
    def _get_error_summary(self) -> Dict:
        ...
        for error in self.errors:
            error_summary["error_types"][error.error_type] = (
                error_summary["error_types"].get(error.error_type, 0) + 1
            )

        # Get most common errors
        sorted_errors = sorted(
            error_summary["error_types"].items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _log_warning(self, message: str, context: Dict = None):
        ...
        logger.warning(
            f"[BulkCSVGenerator] {message} | Context: {json.dumps(log_data)}"
        )
    ...
    def _log_performance(self, operation: str, duration: float, context: Dict = None):
        ...
        logger.info(
            f"[BulkCSVGenerator] Performance: {operation} took {duration:.2f}s | Context: {json.dumps(log_data)}"
        )
        ...
        if self.unified_progress and self.job_id:
            try:
                ...
                if process and process.status.value == "cancelled":
                    logger.info(
                        f"Job {self.job_id} detected as cancelled in progress system"
                    )
                    self.cancelled = True
                    return True
            except Exception as e:
    ...
    def _update_progress(
        ...
        logger.info(
            f"PROGRESS_DEBUG_V2: _update_progress entry - message='{message[:50]}...', percentage={percentage}, is_complete={is_complete}"
        )
        if self.unified_progress and self.job_id:
            try:
                if is_complete:
                    ...
                    if percentage > 0 and percentage < 1:
                        ...
                        progress_value = math.ceil(
                            percentage
                        )  # Round UP to show progress
                    progress_value = min(100, progress_value)  # Cap at 100%
                    logger.info(
                        f"PROGRESS_DEBUG: _update_progress called with percentage={percentage:.2f}, sending progress_value={progress_value}"
                    )
                    ...
                    if hasattr(self, "_current_tasks") and self._current_tasks:
                        ...
                        hasattr(self, "_current_tasks_count")
                        and self._current_tasks_count
                    ):
                        try:
                    ...
                    calculated_progress = (
                        min(100, max(0, int(generated * 100 / total_items)))
                        if total_items > 0
                        else 0
                    )
                    ...
                    logger.info(
                        f"CSV Progress: {generated}/{total_items} items ({calculated_progress}%) - {message}"
                    )
                logger.info(
                    f"Progress updated: {message} ({percentage if percentage else 'calculated'}%)"
                )
            except Exception as e:
    ...
    def _update_task_status(self, status: str, message: str = None):
        ...
        try:
            ...
            if task:
                ...
                db.session.commit()
                logger.info(
                    f"Updated task {task.id} status to '{status}' for job {self.job_id}"
                )
            else:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to rollback database session in _update_task_status: {rollback_error}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to remove database session in _update_task_status: {remove_error}"
                    )
    ...
    def _update_task_status_with_context(self, status: str, message: str = None):
        ...
        try:
            ...
            logger.warning(
                f"Failed to update task status in context for job {self.job_id}: {e}"
            )
            try:
                db.session.rollback()
            except Exception as rollback_error:
        ...
        if self.unified_progress and self.job_id:
            try:
                self.unified_progress.error_process(
                    process_id=self.job_id, message=error_message
                )
    ...
    def _create_unified_prompt(self, task) -> str:
        ...
        if hasattr(task, "prompt_rule_id") and task.prompt_rule_id:
            try:
                ...
                rule = (
                    ...
                    .filter(Rule.id == task.prompt_rule_id, Rule.is_active == True)
                    .first()
                )

                if rule and rule.rule_text:
                    logger.info(
                        f"Using Rule #{rule.id}: {rule.name} for task {task.item_id}"
                    )
                    return self._inject_placeholders(rule.rule_text, task)
                else:
                    logger.warning(
                        f"Rule #{task.prompt_rule_id} not found or inactive, trying default rule"
                    )
            except Exception as e:
                logger.error(
                    f"Error loading rule #{task.prompt_rule_id}: {e}", exc_info=True
                )
        ...
        try:
            ...
            default_rule_name = (
                f"{output_format}_gen_default"  # e.g., "csv_gen_default"
            )
            ...
            if context_mode == "enhanced":
                default_rule_name = (
                    f"{output_format}_gen_enhanced"  # e.g., "csv_gen_enhanced"
                )
            ...
            if rule_text and rule_id:
                logger.info(
                    f"Using default rule '{default_rule_name}' (ID: {rule_id}) for task {task.item_id}"
                )
                return self._inject_placeholders(rule_text, task)
            else:
                logger.info(
                    f"No default rule '{default_rule_name}' found, using hardcoded fallback"
                )
        ...
        phone = (
            getattr(task, "phone", None) or getattr(task, "client_phone", None) or ""
        )
        ...
        hours = (
            getattr(task, "hours", None) or getattr(task, "business_hours", None) or ""
        )
        ...
        if brand_voice_examples and brand_voice_examples.strip():
            business_context_parts.append(
                f"Brand Voice Example: {brand_voice_examples[:200]}"
            )  # Limit length
        ...
        if regulatory_constraints and regulatory_constraints.strip():
            business_context_parts.append(
                f"Compliance Requirements: {regulatory_constraints}"
            )
...
Generate the CSV row now:"""
    ...
    def _inject_placeholders(self, rule_text: str, task) -> str:
        ...
        competitor_urls_str = (
            ", ".join(competitor_urls_list) if competitor_urls_list else ""
        )
    ...
    def _parse_csv_response(
        self, response: str, task: GenerationTask
    ) -> Optional[ContentRow]:
        ...
        try:
            ...
            if csv_lines:
                response_clean = "\n".join(csv_lines)
                logger.info(
                    f"Extracted {len(csv_lines)} CSV line(s) from response (stripped preamble)"
                )
            ...
            if len(fields) <= 1:
                for row in reader:
                    if len(row) >= 2:
                        logger.info(
                            f"Skipped preamble row ('{fields[0][:50]}...'), using next row with {len(row)} fields"
                        )
                        fields = row
                        break
            ...
            task_id = (
                task.item_id
                if hasattr(task, "item_id") and task.item_id
                else f"task_{int(time_module.time() * 1000) % 10000:04d}"
            )
            ...
            if llm_generated_id and llm_generated_id != task_id:
                if llm_generated_id.startswith("task_"):
                    logger.warning(
                        f"FAILSAFE: LLM generated incorrect ID '{llm_generated_id}', forcing to '{task_id}'"
                    )
                else:
                    logger.info(
                        f"Using task_id: {task_id} (LLM provided: {llm_generated_id})"
                    )
            else:
                logger.info(
                    f"Using task_id: {task_id} from task.item_id: {getattr(task, 'item_id', 'MISSING')}"
                )
            title = safe_get_field(
                1, task.topic[:100] if task.topic else "Generated Content"
            )
            ...
            fallback_tags = ", ".join(
                [word.strip(".,!?") for word in topic_words if len(word) > 3]
            )
            tags = safe_get_field(
                5, fallback_tags if fallback_tags else "professional, services"
            )
            ...
            if len(fields) > 7 and len(fields) >= 3:
                ...
                if search_end > 2:  # Ensure we have a valid range
                    ...
                    logger.info(
                        f"Using field {longest_field_idx} as content (length: {len(content)})"
                    )
            ...
            if (
                ...
                or len(content_stripped) < 200
                or word_count_early < 30
            ):
                logger.error(
                    f"Content too short for task {task_id}: {len(content_stripped)} chars, {word_count_early} words (minimum: 200 chars, 30 words)"
                )
                return None
            ...
            if ("|" in category or "," in category) and not (
                "|" in tags or "," in tags
            ):
                logger.warning(
                    f"AUTO-CORRECT: Swapping Category '{category[:50]}...' with Tags '{tags[:50]}...'"
                )
            ...
            if len(excerpt) < 50 and len(category) > 100:
                logger.warning(
                    f"AUTO-CORRECT: Swapping short Excerpt with long Category"
                )
            ...
            if len(content) < 100 and len(excerpt) > 200:
                logger.warning(
                    f"AUTO-CORRECT: Swapping short Content ({len(content)} chars) with long Excerpt ({len(excerpt)} chars)"
                )
            ...
            if "|" in tags:
                tags = tags.replace("|", ", ")
                logger.info(
                    f"AUTO-CORRECT: Converted pipe separators to commas in tags"
                )
            ...
            if not excerpt or len(excerpt.strip()) < 50:
                ...
                logger.info(
                    f"AUTO-CORRECT: Generated excerpt from content ({len(excerpt)} chars)"
                )
            ...
            if category:
                ...
                if category.startswith("[") and category.endswith("]"):
                    logger.warning(
                        f"AUTO-CORRECT: Removing JSON array formatting from category: {category}"
                    )
                ...
                if "-" in category or category.endswith(")"):
                    ...
                    if hyphen_count >= 3 or category.endswith(")"):
                        logger.warning(
                            f"AUTO-CORRECT: Category '{category}' looks like a slug, regenerating..."
                        )
                ...
                if " " not in category and (category.islower() or category.istitle()):
                    ...
                    if len(category) < 15:  # Cities are typically short names
                        logger.warning(
                            f"AUTO-CORRECT: Category '{category}' looks like a city name, regenerating..."
                        )
                        category_invalid = True
                ...
                if category and " " not in category and len(category) > 20:
                    ...
                    if capital_count >= 3:
                        logger.warning(
                            f"AUTO-CORRECT: Category '{category}' appears concatenated ({capital_count} capitals), regenerating..."
                        )
                        category_invalid = True
            ...
            if tags:
                ...
                if len(tag_list) < 3:
                    ...
                    topic_words = [
                        w.strip(".,!?:") for w in task.topic.split() if len(w) > 4
                    ][:3]
                    tag_list.extend(topic_words)
                    tag_list = list(dict.fromkeys(tag_list))[
                        :7
                    ]  # Remove duplicates, max 7
                elif len(tag_list) > 7:
                    tag_list = tag_list[:7]
            ...
            if (
                hasattr(task, "insert_content")
                and task.insert_content
                and hasattr(task, "insert_position")
                and task.insert_position
            ):
                ...
                logger.info(
                    f"Applied insert_content to task {task_id} at position: {task.insert_position}"
                )
        ...
        try:
            ...
            for field_value, field_name in required_fields:
                if not field_value or field_value.strip() == "":
                    logger.warning(
                        f"Missing required field {field_name} in row {content_row.id}"
                    )
                    return False
            ...
            if len(content_row.content.strip()) < 50:
                logger.warning(
                    f"Content too short ({len(content_row.content)} chars) in row {content_row.id}"
                )
                return False

            if len(content_row.content.strip()) > 10000:
                logger.warning(
                    f"Content too long ({len(content_row.content)} chars) in row {content_row.id}"
                )
                return False
            ...
            if content_row.excerpt and len(content_row.excerpt.strip()) > 500:
                logger.warning(
                    f"Excerpt too long ({len(content_row.excerpt)} chars) in row {content_row.id}"
                )
                return False
            ...
            if len(content_row.title.strip()) < 10:
                logger.warning(
                    f"Title too short ({len(content_row.title)} chars) in row {content_row.id}"
                )
                return False

            if len(content_row.title.strip()) > 200:
                logger.warning(
                    f"Title too long ({len(content_row.title)} chars) in row {content_row.id}"
                )
                return False
            ...
            for pattern in error_patterns:
                if pattern.lower() in content_lower:
                    logger.warning(
                        f"Error pattern detected in content: {pattern} in row {content_row.id}"
                    )
                    return False
            ...
            with open(file_path, "r", encoding="utf-8") as csvfile:
                ...
                try:
                    ...
                    if len(headers) != len(expected_headers):
                        logger.error(
                            f"CSV headers count mismatch: expected {len(expected_headers)}, got {len(headers)}"
                        )
                        return False

                    for i, (expected, actual) in enumerate(
                        zip(expected_headers, headers)
                    ):
                        if expected != actual:
                            logger.error(
                                f"CSV header mismatch at position {i}: expected '{expected}', got '{actual}'"
                            )
                            return False
                ...
                for row_num, row in enumerate(
                    reader, start=2
                ):  # Start at 2 because we already read headers
                    row_count += 1

                    if len(row) != len(expected_headers):
                        logger.error(
                            f"Row {row_num} has wrong field count: expected {len(expected_headers)}, got {len(row)}"
                        )
                        return False
                    ...
                    if not row[0].strip() or not row[1].strip() or not row[2].strip():
                        logger.error(
                            f"Row {row_num} has empty required fields: ID='{row[0]}', Title='{row[1]}', Content='{row[2]}'"
                        )
                        return False
                ...
                logger.info(
                    f"CSV validation passed: {file_path} ({row_count} data rows)"
                )
                return True
    ...
    def _validate_job_completion_requirements(
        self, file_path: str, expected_tasks: int
    ) -> bool:
        """Validate that the job meets completion requirements before marking as complete"""
        try:
            if not os.path.exists(file_path):
                logger.error(
                    f"Job completion validation failed: File does not exist: {file_path}"
                )
                return False
            ...
            if file_size == 0:
                logger.error(
                    f"Job completion validation failed: File is empty: {file_path}"
                )
                return False
            ...
            with open(file_path, "r", encoding="utf-8") as csvfile:
                ...
                try:
                    ...
                    logger.error(
                        f"Job completion validation failed: No headers found in file: {file_path}"
                    )
                    return False
                ...
                logger.info(
                    f"Job completion validation: Found {data_row_count} valid data rows, expected {expected_tasks} tasks"
                )

                # Check if we have a reasonable number of successful generations
                success_rate = (
                    data_row_count / expected_tasks if expected_tasks > 0 else 0
                )
                ...
                if success_rate < min_success_rate:
                    logger.error(
                        f"Job completion validation failed: Success rate too low ({success_rate:.1%}) - only {data_row_count}/{expected_tasks} items generated (minimum: {min_success_rate:.1%})"
                    )
                    return False

                if data_row_count == 0:
                    logger.error(
                        f"Job completion validation failed: No valid data rows generated"
                    )
                    return False
                ...
                for row_num, row in enumerate(reader, start=2):
                    if len(row) >= 3:
                        ...
                        if len(content) < 30:  # More lenient minimum content length
                            ...
                            len(content) > 15000
                        ):  # More lenient maximum content length
                            quality_issues += 1

                quality_rate = (
                    (data_row_count - quality_issues) / data_row_count
                    if data_row_count > 0
                    else 0
                )
                if (
                    quality_rate < 0.6
                ):  # More lenient quality requirement - require at least 60% quality rate
                    logger.error(
                        f"Job completion validation failed: Quality rate too low ({quality_rate:.1%}) - {quality_issues} quality issues found (minimum: 60%)"
                    )
                    return False

                logger.info(
                    f"Job completion validation passed: {data_row_count} valid rows, {success_rate:.1%} success rate, quality rate acceptable"
                )
                return True
    ...
    def _write_csv_with_conversion(self, rows: List[Dict], output_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    f"Failed to write CSV file with both methods: {fallback_error}"
                )
    ...
    def _validate_content_row(self, content_row: ContentRow) -> bool:
        ...
        try:
            ...
            if not content_row.content or len(content_row.content.strip()) < 50:
                self._log_warning(
                    f"Content row {content_row.id} has insufficient content"
                )
                return False
            ...
            if any(content.endswith(ending) for ending in incomplete_endings):
                self._log_warning(
                    f"Truncated HTML detected for row {content_row.id}: ends with incomplete tag"
                )
                return False
            ...
            if open_tags > 0 and close_tags == 0:
                self._log_warning(
                    f"Malformed HTML for row {content_row.id}: {open_tags} open tags, {close_tags} close tags"
                )
                return False
            ...
            for pattern in error_patterns:
                if pattern in lower_content[:200]:  # Check first 200 chars
                    self._log_warning(
                        f"Content appears to be metadata/error for row {content_row.id}: contains '{pattern}'"
                    )
                    return False
            ...
            if content_row.slug and not is_valid_slug(content_row.slug):
                self._log_warning(
                    f"Content row {content_row.id} has invalid slug format: {content_row.slug}"
                )
    ...
    def _generate_single_content(self, task: GenerationTask) -> Optional[ContentRow]:
        ...
        try:
            ...
            logger.info(
                f"Generating content for task {task.item_id} with prompt length: {len(prompt)}"
            )
            ...
            try:
                ...
                if (
                    hasattr(task, "previous_attempt_failed_reason")
                    and task.previous_attempt_failed_reason
                ):
                    ...
                    logger.info(
                        f"Enhanced system prompt with retry feedback: {task.previous_attempt_failed_reason}"
                    )
                ...
                if (
                    ...
                    logger.info(
                        f"Using llm_response.message.content: {len(response_text) if response_text else 0} chars"
                    )
                else:
                    response_text = str(llm_response) if llm_response else ""
                    logger.info(
                        f"Using str(llm_response): {len(response_text) if response_text else 0} chars"
                    )
                ...
                if not response_text or len(response_text.strip()) == 0:
                    ...
                    logger.error(
                        f"  - hasattr message: {hasattr(llm_response, 'message') if llm_response else 'N/A'}"
                    )
                    if llm_response and hasattr(llm_response, "message"):
                        ...
                        if llm_response.message:
                            try:
                                logger.error(
                                    f"  - message.content: '{llm_response.message.content}'"
                                )
                            except (ValueError, AttributeError):
                                blocks = getattr(llm_response.message, "blocks", [])
                                _debug_content = next(
                                ...
                                logger.error(
                                    f"  - message.content (from blocks): '{_debug_content}'"
                                )
                    return None

            except Exception as e:
                logger.error(
                    f"LLM chat() failed for task {task.item_id}: {e}", exc_info=True
                )
                return None
            ...
            logger.info(
                f"Response text preview: {response_text[:100] if response_text else 'EMPTY'}"
            )
            ...
            if error_detection:
                ...
                self._record_error(
                    ...
                    error_type,
                    error_description,
                    f"Error pattern detected in response: {response_text[:200]}...",
                    can_retry=True,
                )
                return None
            ...
            if not content_row:
                self._record_error(
                    ...
                    f"Could not parse CSV from response: {response_text[:200]}...",
                    can_retry=True,
                )
                return None
            ...
            if not self._validate_content_row(content_row):
                self._record_error(
                    task.item_id,
                    "validation_failed",
                    "Content row validation failed",
                    f"Row failed validation: {content_row.id}",
                    can_retry=True,
                )
                return None
            ...
            if word_count < minimum_words:
                ...
                self._record_error(
                    ...
                    "content_too_short",
                    error_msg,
                    f"Generated {word_count} words, required minimum {minimum_words}",
                    can_retry=True,
                )
                ...
                word_count < self.target_word_count * 1.1
            ):  # Warn if just barely meeting minimum
                logger.warning(
                    f"Content word count ({word_count}) meets minimum but is below ideal for task {task.item_id}"
                )

            logger.info(
                f"Generated content for task {task.item_id}: {word_count} words"
            )
            return content_row

        except Exception as e:
            logger.error(
                f"Error generating content for task {task.item_id}: {e}", exc_info=True
            )
            return None

    def _create_generation_record(
        self, tasks: List[GenerationTask], site_key: str = None
    ) -> str:
        ...
        try:
            ...
            self._log_warning(
                "Persistence layer not yet implemented - skipping generation record"
            )
            return None
    ...
    def _persist_page(self, content_row: ContentRow, generation_id: str) -> bool:
        ...
        try:
            ...
            self._log_warning(
                f"Persistence layer not implemented - skipping page {content_row.id}"
            )
            return False
    ...
    def _generate_single_content_with_retry(
        ...
        if self.circuit_breaker_active:
            ...
            self._record_error(
                task.item_id,
                "circuit_breaker",
                "Circuit breaker active due to consecutive failures",
                can_retry=False,
            )
            return None
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.info(
                    f"Attempt {attempt + 1}/{max_retries + 1} for task {task.item_id}"
                )
                ...
                if result:
                    ...
                    if self.circuit_breaker_active and self.successful_operations >= 3:
                        self.circuit_breaker_active = False
                        logger.info(
                            f"Circuit breaker reset after {self.successful_operations} successful operations"
                        )

                    if attempt > 0:
                        logger.info(
                            f"Task {task.item_id} succeeded on attempt {attempt + 1}"
                        )
                    return result
                else:
                    if attempt < max_retries:
                        ...
                        retryable_errors = [
                            error
                            for error in self.errors
                            if error.task_id == task.item_id and error.can_retry
                        ]
                        if retryable_errors:
                            logger.warning(
                                f"Task {task.item_id} failed on attempt {attempt + 1}, retrying... (retryable error)"
                            )
                            time.sleep(1 * (attempt + 1))  # Exponential backoff
                            continue
                        else:
                            logger.error(
                                f"Task {task.item_id} failed with non-retryable error, not retrying"
                            )
                            break
                    else:
                        logger.error(
                            f"Task {task.item_id} failed after {max_retries + 1} attempts"
                        )
                        break

            except Exception as e:
                logger.error(
                    f"Exception on attempt {attempt + 1} for task {task.item_id}: {e}"
                )
                if attempt < max_retries:
                    time.sleep(1 * (attempt + 1))  # Exponential backoff
                    continue
        ...
        if self.consecutive_failures >= self.max_consecutive_failures:
            ...
            logger.error(
                f"Circuit breaker activated after {self.consecutive_failures} consecutive failures"
            )
            ...
            if self.unified_progress and self.job_id:
                try:
                    ...
                    logger.warning(
                        f"Failed to update progress with circuit breaker error: {e}"
                    )

        return None

    def _generate_batch_concurrent(
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    result = future.result(
                        timeout=120
                    )  # FIXED: 2 minute timeout per task
                    if result:
                        ...
                        logger.info(
                            f"PROGRESS_DEBUG: current_page={current_page}, total_pages={total_pages}, percentage={progress_percentage:.2f}%"
                        )
                        ...
                        self.error_count += 1
                        logger.error(
                            f"Failed to generate content for task {task.item_id}"
                        )
                except Exception as e:
                    self.error_count += 1
    ...
    def _chunk_large_task_set(
        self, tasks: List[GenerationTask], chunk_size: int = 100
        ...
        self._log_info(
            f"Split {len(tasks)} tasks into {len(chunks)} chunks of max {chunk_size} tasks each"
        )
        return chunks

    def generate_bulk_csv_with_web_research(
        self,
        ...
        self._log_info(
            f"Starting large-scale CSV generation: {total_tasks} tasks with web research: {enable_web_research}"
        )
        ...
        if total_tasks > 500:
            ...
            for chunk_index, chunk_tasks in enumerate(task_chunks, 1):
                self._log_info(
                    f"Processing chunk {chunk_index}/{len(task_chunks)} ({len(chunk_tasks)} tasks)"
                )

                # Process chunk with web research
                chunk_results = self._process_chunk_with_research(
                    chunk_tasks, enable_web_research, max_sources_per_task, chunk_index
                )
                ...
                completed_tasks = len(
                    all_results
                )  # More accurate count based on actual results
                progress = (completed_tasks / total_tasks) * 100
                self._update_progress(
                    f"Completed chunk {chunk_index}/{len(task_chunks)} - {completed_tasks}/{total_tasks} tasks",
                    progress,
                )
    ...
    def _process_chunk_with_research(
        self,
        tasks: List[GenerationTask],
        ...
        for task in tasks:
            try:
                ...
                if enable_web_research and WEB_SEARCH_AVAILABLE:
                    research_context = self._gather_web_research(
                        task.topic, max_sources_per_task
                    )
                ...
                self._log_error(
                    f"Failed to process task {task.item_id} in chunk {chunk_index}: {e}"
                )
                continue
    ...
    def _write_enhanced_csv(
        self, content_rows: List[ContentRow], output_filename: str
    ) -> Tuple[str, Dict]:
            ...
            csv_data.append(
                {
                    ...
                    "Tags": row.tags
                    or "professional, services, quality, expert, trusted",
        ...
        stats = {
            ...
            "success_rate": (
                len(csv_data) / (len(csv_data) + self.error_count) * 100
                if (len(csv_data) + self.error_count) > 0
                else 0
            ),
        ...
        self._log_info(
            f"Enhanced CSV generation complete: {output_path} with {len(csv_data)} rows"
        )
    ...
    def generate_bulk_csv(
        ...
        logger.info(
            f"[DEBUG] generate_bulk_csv START in thread '{thread_name}' with {len(tasks)} tasks"
        )
        ...
        if resume_from_id:
            ...
            logger.info(
                f"Resuming from task {resume_from_id}, {len(tasks)} tasks remaining"
            )
        ...
        self.row_tracker.export_tracking_log_json(
            json_path, initial_job_params, status="in_progress"
        )
        ...
        while generation_round < max_rounds:
            ...
            if len(active_rows) >= target_count:
                self._log_info(
                    f"Target count reached: {len(active_rows)}/{target_count}"
                )
                break
            ...
            if not needs_work:
                ...
                flagged_rows = [
                    r
                    for r in self.row_tracker.records.values()
                    if r.status == RowStatus.FLAGGED
                ]
                if len(active_rows) + len(flagged_rows) >= target_count:
                    ...
                    self._log_error(
                        "Cannot reach target count - insufficient valid rows"
                    )
                    break
            ...
            for seq_num, unique_id in batch:
                    ...
                    if record.replacement_history:
                        ...
                        if last_failure:
                            ...
                            if last_failure.word_count:
                                task.previous_attempt_word_count = (
                                    last_failure.word_count
                                )
                ...
                logger.info(
                    f"[DEBUG] About to generate content for seq_num={seq_num}, task={task.item_id}"
                )
                self._update_progress(
                ...
                logger.info(
                    f"[DEBUG] Calling _generate_single_content for task {task.item_id}"
                )

                try:
                    content_row = self._generate_single_content(task)
                    logger.info(
                        f"[DEBUG] _generate_single_content returned for task {task.item_id} (result: {'OK' if content_row else 'None'})"
                    )
                    ...
                    if validation_passed:
                        ...
                        row_data = {
                            ...
                            or "professional, services, quality",
                            "slug": content_row.slug
        ...
        self.row_tracker.export_tracking_log_json(
            self._tracking_json_path, job_params, status="completed"
        )
        logger.info(
            f"Updated tracking JSON with final results: {self._tracking_json_path}"
        )
        ...
        if all_content_rows:
            logger.info(
                f"  - Sample content row keys: {list(all_content_rows[0].keys()) if all_content_rows[0] else 'None'}"
            )
            logger.info(
                f"  - Sample content row values: {list(all_content_rows[0].values())[:3] if all_content_rows[0] else 'None'}"
            )
        else:
            ...
            logger.error(
                f"  - Active records: {sum(1 for r in self.row_tracker.records.values() if r.is_active)}"
            )
            for record in self.row_tracker.records.values():
                if not record.is_active:
                    logger.error(
                        f"    - Inactive record {record.unique_id}: status={record.status}, replacement_count={record.replacement_count}"
                    )
        ...
        if all_content_rows:
            try:
                logger.info(
                    f"Writing {len(all_content_rows)} rows with pipe-to-comma conversion"
                )
                self._write_csv_with_conversion(all_content_rows, output_path)
                logger.info(
                    f"Successfully wrote CSV file with conversion: {output_path}"
                )
            except Exception as e:
        ...
        if os.path.exists(output_path):
            ...
            if not self._validate_job_completion_requirements(output_path, len(tasks)):
                logger.error(
                    f"Job completion validation failed for file: {output_path}"
                )
                # Remove invalid file
                os.remove(output_path)
                ...
                completion_error_message = (
                    f"Generation failed: Job completion requirements not met"
                )
                self._update_progress_with_error(completion_error_message)

                raise Exception(
                    "CSV generation failed - job completion requirements not met"
                )

            logger.info(
                f"Generated and validated CSV file: {output_path} ({file_size} bytes)"
            )
        ...
        stats = {
            ...
            "success_rate": (
                actual_generated_count / len(tasks) * 100 if len(tasks) > 0 else 0
            ),
            "total_time_seconds": total_time,
            "average_time_per_item": (
                total_time / actual_generated_count if actual_generated_count > 0 else 0
            ),
            "items_per_hour": (
                actual_generated_count / (total_time / 3600) if total_time > 0 else 0
            ),
            ...
            "tracking_files": {"json_log": json_path},
        }

        logger.info(f"Bulk generation complete: {stats}")
        return output_path, stats

    def generate_retry_csv(
        self, tasks: List[GenerationTask], output_filename: str
    ) -> Tuple[str, Dict]:
        """
        ...
        try:
            ...
            retry_batch_size = min(
                len(tasks), 10
            )  # Process all items in 1-2 batches max
            ...
            logger.info(
                f"Retry optimization: batch_size={retry_batch_size}, workers={retry_workers}"
            )
...
def create_tasks_from_topics(
    topics: List[str], client: str, project: str, website: str, client_notes: str = None
) -> List[GenerationTask]:
    ...
    try:
        max_page = (
            ...
            .filter(Page.meta_json.like(f"%{website_key}%"))
...
def create_demonstration_csv(
    output_dir: str, num_items: int = 20, concurrent_workers: int = 5
) -> Tuple[str, Dict]:
    """
...
Brand Tones:
  neutral, friendly, authoritative, playful, luxury
    ...
    # Site metadata
    parser.add_argument(
        "--company-name", type=str, help="Company name for content generation"
    )
    parser.add_argument("--phone", type=str, help="Phone number for content generation")
    parser.add_argument(
        "--contact-url", type=str, help="Contact URL for content generation"
    )
    parser.add_argument(
        "--client-location", type=str, help="Client location for content generation"
    )
    parser.add_argument(
        "--primary-service", type=str, help="Primary service for content generation"
    )
    parser.add_argument(
        "--secondary-service", type=str, help="Secondary service for content generation"
    )
    parser.add_argument(
        "--brand-tone",
        choices=["neutral", "friendly", "authoritative", "playful", "luxury"],
        default="neutral",
        help="Brand tone for content generation (default: neutral)",
    )
    parser.add_argument(
        "--hours", type=str, help="Business hours for content generation"
    )
...
def main():
    ...
    try:
        ...
        if stats["error_count"] > 0:
            ...
            if (
                "error_summary" in stats
                and "most_common_errors" in stats["error_summary"]
            ):
                print(
                    f"  - Most common errors: {stats['error_summary']['most_common_errors'][:3]}"
                )
