...
try:
    ...
    try:
        ...
        logger.warning(
            f"Chunk metadata cache: Redis not available, using in-memory cache: {e}"
        )
        USE_REDIS = False
        _redis_client = None
except ImportError:
    logger.warning(
        "Chunk metadata cache: Redis library not available, using in-memory cache"
    )
    USE_REDIS = False
    _redis_client = None
...
def set_cached_metadata(
    document_id: str, content_hash: str, metadata: Dict[str, Any], ttl: int = 86400
) -> bool:
    """
    ...
    try:
        ...
        if USE_REDIS and _redis_client:
            _redis_client.setex(cache_key, ttl, json.dumps(metadata, default=str))
            _cache_stats["sets"] += 1
            return True
        else:
        ...
        if USE_REDIS and _redis_client:
            ...
            while True:
                cursor, keys = _redis_client.scan(
                    cursor, match="chunk_metadata:*", count=100
                )
                if keys:
                    _redis_client.delete(*keys)
                    count += len(keys)
