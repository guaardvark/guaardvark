...
class ConversationLogger:
    ...
    def log_exchange(
        ...
        logger.debug(
            f"Logged exchange for session {session_id}: {len(user_message)} chars user, {len(assistant_response)} chars assistant"
        )
    ...
    def _generate_session_summary(self, session: ConversationSession) -> str:
        ...
        question_types = self._analyze_question_types(
            [entry.user_message for entry in session.entries]
        )
        response_patterns = self._analyze_response_patterns(
            [entry.assistant_response for entry in session.entries]
        )

        # Calculate enhanced statistics
        total_user_tokens = sum(
            entry.token_count // 2 for entry in session.entries
        )  # Estimate user portion
        ...
        for i, entry in enumerate(session.entries, 1):
            ...
            user_preview = (
                entry.user_message[:100] + "..."
                if len(entry.user_message) > 100
                else entry.user_message
            )
            response_preview = (
                entry.assistant_response[:150] + "..."
                if len(entry.assistant_response) > 150
                else entry.assistant_response
            )
            ...
            summary_lines.extend(
                [
                    ...
                    (
                        f"- **Efficiency**: {entry.token_count/entry.processing_time:.1f} tokens/sec"
            ...
            if entry.context_used:
                summary_lines.append(
                    f"- **Context Used**: {', '.join(entry.context_used[:3])}{'...' if len(entry.context_used) > 3 else ''}"
                )
            ...
            if entry.metadata:
                important_metadata = {
                    ...
                    if k
                    in ["project_id", "document_analyzed", "error_occurred", "rag_used"]
                }
            ...
            if i >= 15:
                ...
                summary_lines.append(
                    f"... and {len(session.entries) - 15} more exchanges"
                )
                break
    ...
    def _analyze_question_types(self, user_messages: List[str]) -> Dict[str, int]:
        ...
        for message in user_messages:
            ...
            if any(
                phrase in message_lower for phrase in ["how to", "how do", "how can"]
            ):
                question_types["how_to"] += 1
            elif any(
                phrase in message_lower
                for phrase in ["what is", "what are", "what does"]
            ):
                question_types["what_is"] += 1
            elif any(
                phrase in message_lower
                for phrase in ["error", "bug", "fix", "debug", "problem"]
            ):
                question_types["debugging"] += 1
            elif any(
                phrase in message_lower for phrase in ["explain", "why", "understand"]
            ):
                question_types["explanation"] += 1
            elif any(
                phrase in message_lower
                for phrase in ["implement", "create", "build", "add"]
            ):
    ...
    def _analyze_response_patterns(
        ...
        for response in assistant_responses:
            ...
            if "```" in response or "code" in response_lower:
                ...
                for phrase in ["step 1", "first", "then", "next", "finally"]
            ):
                patterns["step_by_step"] += 1
            elif any(
                phrase in response_lower
                for phrase in ["error", "fix", "problem", "issue"]
            ):
                patterns["error_help"] += 1
            elif any(
                phrase in response_lower
                for phrase in ["recommend", "suggest", "consider", "best practice"]
            ):
    ...
    def _format_question_types(self, question_types: Dict[str, int]) -> str:
        ...
        for q_type, count in question_types.items():
            if count > 0:
                ...
                formatted.append(
                    f"{q_type.replace('_', ' ').title()}: {count} ({percentage:.1f}%)"
                )
    ...
    def _assess_conversation_complexity_from_entries(
        ...
        avg_processing_time = sum(entry.processing_time for entry in entries) / len(
            entries
        )
    ...
    def _find_most_active_period_from_entries(
        self, entries: List[ConversationEntry]
    ) -> str:
    ...
    def _assess_user_engagement_from_entries(
        self, entries: List[ConversationEntry]
    ) -> str:
        ...
        avg_user_message_length = sum(
            len(entry.user_message) for entry in entries
        ) / len(entries)
        ...
        try:
            ...
            if not session_dir.exists():
                try:
                    ...
                    logger.error(
                        f"Failed to create sessions directory {session_dir}: {dir_error}"
                    )
                    return
                ...
                for i, entry in enumerate(session.entries):
                    try:
                        ...
                        if isinstance(entry_dict.get("timestamp"), datetime):
                            entry_dict["timestamp"] = entry_dict[
                                "timestamp"
                            ].isoformat()
                        session_data["entries"].append(entry_dict)
                    except Exception as entry_error:
                        logger.warning(
                            f"Failed to serialize entry {i} for session {session_id}: {entry_error}"
                        )
                        continue

            except Exception as data_error:
                logger.error(
                    f"Failed to prepare session data for {session_id}: {data_error}"
                )
                return
            ...
            try:
                with open(temp_file, "w", encoding="utf-8") as f:
                    json.dump(
                        session_data, f, indent=2, default=str, ensure_ascii=False
                    )
                ...
                logger.error(
                    f"Failed to write session file for {session_id}: {write_error}"
                )
            ...
            logger.error(
                f"Failed to save session incrementally for {session_id}: {e}",
                exc_info=True,
            )
    ...
    def _save_complete_session(self, session: ConversationSession) -> None:
        ...
        summary_file = (
            self.storage_dir / "summaries" / f"{session.session_id}_summary.md"
        )
        ...
        try:
            ...
            active_file = (
                self.storage_dir / "sessions" / f"{session.session_id}_active.json"
            )
...
"""
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_entries": sum(
                len(session.entries) for session in self.active_sessions.values()
            ),
            "total_tokens": sum(
