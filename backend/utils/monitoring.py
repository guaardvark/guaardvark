...
class MetricsCollector:
    ...
    def get_metrics_history(
        ...
        with self._lock:
            ...
            return [
                sample
                for sample in self.metrics_history[metric_name]
        ...
        try:
            ...
            if current_metrics.cpu_percent >= cpu_critical:
                health_issues.append(
                    f"Critical CPU usage: {current_metrics.cpu_percent:.1f}%"
                )
                status = "critical"
            ...
            if current_metrics.disk_usage_percent >= disk_critical:
                health_issues.append(
                    f"Critical disk usage: {current_metrics.disk_usage_percent:.1f}%"
                )
                status = "critical"
            ...
            if high_error_endpoints:
                health_issues.append(
                    f"High error rate on endpoints: {', '.join(high_error_endpoints)}"
                )
                if status == "healthy":
                    status = "warning"
            ...
            return {
                ...
                "api_summary": {
                    ...
                    "total_requests": sum(
                        m["request_count"] for m in api_metrics.values()
                    ),
