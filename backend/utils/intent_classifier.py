...
try:
    from backend.services.intent_service import (
        get_intent_classifier as get_semantic_classifier,
    )
...
class IntentClassifier:
    ...
    def classify_intent(self, message: str) -> Tuple[IntentType, float, Dict]:
        ...
        db_confidence, db_keywords = self._check_keywords(
            message_lower, self.database_keywords
        )
        ...
        doc_confidence, doc_keywords = self._check_keywords(
            message_lower, self.document_keywords
        )
        ...
        if _semantic_classifier is not None:
            try:
                semantic_intent, semantic_confidence = _semantic_classifier.classify(
                    message
                )
                ...
                if semantic_intent == "realtime" and semantic_confidence > 0.6:
                    ...
                    logger.info(
                        f"Intent: WEB_SEARCH detected (semantic) - confidence: {semantic_confidence:.2f}"
                    )
                    return IntentType.WEB_SEARCH, semantic_confidence, metadata

            except Exception as e:
                logger.warning(
                    f"Semantic classification failed: {e}, falling back to keywords"
                )
                metadata["classifier_type"] = "keyword"

        # 5. Keyword-based Web Search Detection (fallback)
        web_confidence, web_keywords = self._check_keywords(
            message_lower, self.web_keywords
        )

        # Filter out casual "today" usage
        is_casual_today = any(
            pattern in message_lower for pattern in self.casual_today_patterns
        )
        if web_confidence > 0.4 and not is_casual_today:
            metadata["keywords_found"] = web_keywords
            logger.info(
                f"Intent: WEB_SEARCH detected (keywords) - keywords: {web_keywords}"
            )
...
def get_classifier_status() -> Dict:
    ...
    status = {"keyword_classifier": "active", "semantic_classifier": "not_loaded"}

    if _semantic_classifier is not None:
        try:
            model_info = _semantic_classifier.get_model_info()
            status["semantic_classifier"] = (
                "active" if model_info.get("model_loaded") else "keyword_fallback"
            )
