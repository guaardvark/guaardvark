...
class EnhancedContextCSVGenerator:
    ...
    async def generate_enhanced_csv(
        ...
        try:
            ...
            result = await self._generate_with_full_context(
                tasks, context, output_filename
            )
            ...
            return {"success": False, "error": str(e), "context_gathered": False}
    ...
    async def _build_enhanced_context(
        ...
        if project_name:
            project = Project.query.filter_by(
                client_id=client_id, name=project_name
            ).first()

        logger.info(
            f"Building context for client: {client.name}, project: {project_name}"
        )
    ...
    def _analyze_entity_relationships(
        self, client: Client, project: Project = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add system overview
            system_overview = (
                self.relationship_indexer.create_system_overview_document()
            )
    ...
    async def _analyze_competitor_content(self, competitor_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            products = self._extract_products_from_content(
            ...
            for doc in client_docs:
                try:
                    ...
                    doc_analysis = {
                        ...
                        "content_summary": self._summarize_document_content(
                            query_result
                        ),
    ...
    def _build_industry_context(
        self, client: Client, client_documents: List[Dict]
    ) -> str:
    ...
    def _build_content_strategy(
        self, client: Client, competitor_content: Dict[str, Any], industry_context: str
    ) -> Dict[str, Any]:
    ...
    def _generate_intelligent_topics(
        self, context: EnhancedGenerationContext, num_pages: int
    ) -> List[str]:
        ...
        all_topics = (
            base_topics + competitor_topics + industry_topics + strategic_topics
        )
        return all_topics[:num_pages]  # Ensure exact count

    def _create_enhanced_tasks(
        self, context: EnhancedGenerationContext, topics: List[str]
    ) -> List[GenerationTask]:
    ...
    async def _generate_with_full_context(
        ...
        result = await self.bulk_generator.generate_bulk_csv_async(
            ...
            or f"enhanced_{context.client_data['name']}_content.csv",
    ...
    def _determine_content_angle(
        self, business_type: str, competitor_focus: str
    ) -> str:
    ...
    def _generate_base_topics(self, client_data: Dict, count: int) -> List[str]:
        ...
        return [
            f"Professional {client_data['name']} Solutions Topic {i}"
