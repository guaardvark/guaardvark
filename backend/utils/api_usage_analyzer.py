...
def categorize_apis(blueprints, frontend_usage):
    ...
    for bp_name, bp_info in blueprints.items():
        ...
        # Categorize non-frontend APIs
            ...
            kw in module_name
            for kw in ["celery", "monitor", "system", "meta", "reboot", "diagnostics"]
        ):
...
def print_results(categories, blueprints):
    ...
    print(f"{'=' * 80}")
    for bp_name, bp_info in sorted(
        categories["frontend_used"], key=lambda x: x[1]["frontend_calls"], reverse=True
    ):
    ...
    print(
        f"  ✅ Used by Frontend: {len(categories['frontend_used'])} ({len(categories['frontend_used'])/total*100:.1f}%)"
    )
    print(
        f"  ⚙️  Internal/Backend: {len(categories['internal_backend'])} ({len(categories['internal_backend'])/total*100:.1f}%)"
    )
    print(
        f"  🧪 Test/Debug: {len(categories['test_debug'])} ({len(categories['test_debug'])/total*100:.1f}%)"
    )
    print(
        f"  ❓ Potentially Unused: {len(categories['potentially_unused'])} ({len(categories['potentially_unused'])/total*100:.1f}%)"
    )
