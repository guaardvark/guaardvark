...
def validate_email(email: str) -> str:
    ...
    if len(email) > MAX_LENGTHS["email"]:
        raise ValidationError(
            f"Email too long. Maximum length is {MAX_LENGTHS['email']}"
        )
...
def validate_filename(filename: str) -> str:
    ...
    if len(filename) > MAX_LENGTHS["filename"]:
        raise ValidationError(
            f"Filename too long. Maximum length is {MAX_LENGTHS['filename']}"
        )
...
def validate_request_size(data: Any, max_size_mb: int = 200) -> None:
    ...
    if size_bytes > max_size_bytes:
        raise ValidationError(
            f"Request data too large. Maximum size is {max_size_mb}MB"
        )
...
def validate_session_id(session_id: str) -> str:
    ...
    if len(session_id) > MAX_LENGTHS["session_id"]:
        raise ValidationError(
            f"Session ID too long. Maximum length is {MAX_LENGTHS['session_id']}"
        )
...
def validate_prompt(prompt: str) -> str:
    ...
    if len(prompt) > MAX_LENGTHS["prompt"]:
        raise ValidationError(
            f"Prompt too long. Maximum length is {MAX_LENGTHS['prompt']}"
        )
