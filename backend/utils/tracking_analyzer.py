...
class TrackingAnalyzer:
        ...
        try:
            if not os.path.exists(self.tracking_path):
                raise FileNotFoundError(
                    f"Tracking file not found: {self.tracking_path}"
                )
    ...
    def analyze_tracking_file(self) -> Dict[str, Any]:
        ...
        zero_attempts = len(
            [
                ...
                if not r.get("is_active", False) and r.get("replacement_count", 0) == 0
            ]
        )
        partial_attempts = len(
            [
                r
                for r in row_records
                if not r.get("is_active", False)
                and 0 < r.get("replacement_count", 0) < 5
            ]
        )
        max_attempts_reached = len(
            [r for r in row_records if r.get("max_attempts_reached", False)]
        )
        ...
        for record in row_records:
            if not record.get("is_active", False) and record.get("replacement_history"):
                for attempt in record["replacement_history"]:
                    if not attempt.get("success", False) and attempt.get(
                        "validation_failure"
                    ):
    ...
    def extract_failed_topics(
        self, retry_mode: str = "all_inactive"
    ) -> List[Dict[str, Any]]:
        ...
        if retry_mode == "all_inactive":
            ...
            candidate_records = [
                r for r in row_records if not r.get("is_active", False)
            ]
        ...
        for record in candidate_records:
            ...
            if record.get("replacement_history"):
                ...
                if not last_attempt.get("success", False) and last_attempt.get(
                    "validation_failure"
                ):
                    failure_reason = last_attempt["validation_failure"].get(
                        "reason", "unknown"
                    )
        ...
        logger.info(
            f"Extracted {len(failed_topics)} failed topics for retry mode '{retry_mode}' (processed {len(candidate_records)}/{len(row_records)} records)"
        )
    ...
    def merge_tracking_files(self, retry_tracking_paths: List[str]) -> List[Dict]:
        ...
        active_rows = [
            record for record in original_rows if record.get("is_active", False)
        ]
        ...
        for retry_path in retry_tracking_paths:
            try:
                ...
                retry_active = [
                    record for record in retry_rows if record.get("is_active", False)
                ]

                logger.info(
                    f"Retry file {retry_path} has {len(retry_active)} active rows"
                )
...
def extract_failed_topics(
    tracking_path: str, retry_mode: str = "all_inactive"
) -> List[Dict[str, Any]]:
