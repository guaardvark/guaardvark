...
class CSVTemplate:
    ...
    ENFOLD_WORDPRESS = [
        ...
        CSVColumn(
            "Content",
            "Main content with basic formatting (H1, H2, H3, bold, italic, links) for Avia Builder",
            True,
        ),
        ...
        CSVColumn(
            "Image",
            "Same as category (will be replaced with category image URL)",
            True,
            200,
        ),
    ]
...
class CSVFormatter:
    ...
    def detect_csv_template(
        ...
        try:
            ...
            logger.warning(
                f"Error processing content/prompt for template detection: {e}"
            )
            content_lower = ""
            prompt_lower = ""
            combined = ""
        ...
        # WordPress/Blog detection
            ...
            keyword in combined for keyword in ["wordpress", "blog", "post", "article"]
        ):
        ...
        # Product/E-commerce detection
            ...
            keyword in combined
            for keyword in ["product", "catalog", "ecommerce", "shop", "store"]
        ):
        ...
        # Business pages detection
            ...
            keyword in combined
            for keyword in ["page", "website", "business", "company", "service"]
        ):
    ...
    def _post_process_enfold_rows(
        ...
        for i, row in enumerate(rows):
            ...
            if "Tags" in headers:
                ...
                if current_tags:
                    tag_list = [
                        tag.strip() for tag in current_tags.split(",") if tag.strip()
                    ]
                    if len(tag_list) < 12:
    ...
    def generate_structured_csv_prompt(
        self, user_request: str, template: List[CSVColumn]
    ) -> str:
        """
...
Generate the complete CSV content now:"""
    ...
    def format_content_as_csv(
        self,
        content: str,
        user_prompt: str = "",
        specified_headers: Optional[List[str]] = None,
    ) -> str:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully formatted CSV with {len(headers)} columns and {len(rows)} data rows"
            )
    ...
    def create_sample_csv(
        self, template_name: str = "general", num_rows: int = 3
    ) -> str:
        """
        ...
        for i in range(num_rows):
            ...
            for col in template:
                if col.name in ["title", "post_title", "page_title", "product_name"]:
                    sample_row.append(
                        f"Sample {col.name.replace('_', ' ').title()} {i+1}"
                    )
                elif col.name in [
                    "content",
                    "post_content",
                    ...
                    sample_row.append(
                        f"This is sample content for item {i+1}. It contains detailed information about the topic."
                    )
                elif col.name in [
                    "category",
                    "post_category",
...
def format_csv_content(
    content: str, user_prompt: str = "", headers: Optional[List[str]] = None
) -> str:
...
def generate_csv_prompt(user_request: str, template_name: str = "auto") -> str:
    ...
    if template_name == "auto":
        ...
        template = formatter.templates.get(
            template_name, formatter.templates["general"]
        )
