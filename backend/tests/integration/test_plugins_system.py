...
def plugins_dir(tmp_path):
    ...
    plugin_json = {
        ...
        "requirements": {"gpu": False, "cuda": False},
...
class TestPluginRegistry:
    ...
    def test_update_plugin_config(self, plugins_dir):
        ...
        # Update config
        success = registry.update_plugin_config(
            "test-plugin", {"enabled": True, "timeout": 60}
        )
...
class TestPluginManager:
    ...
    def test_start_plugin_not_enabled(self, plugins_dir):
        ...
        assert (
            "disabled" in result["error"].lower() or "enable" in result["error"].lower()
        )
...
class TestPluginsAPI:
    ...
    def test_list_plugins_endpoint(self, app, client, plugins_dir):
        ...
        with patch(
            "backend.plugins.plugin_registry.get_plugin_registry"
        ) as mock_registry:
    ...
    def test_update_config_endpoint(self, app, client, plugins_dir):
        ...
        with patch(
            ...
            with patch(
                ...
                response = client.put(
                    "/api/plugins/test-plugin/config", json={"timeout": 120}
                )
