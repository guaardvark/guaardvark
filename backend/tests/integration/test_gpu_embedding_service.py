...
try:
    from plugins.gpu_embedding.client.gpu_embedding_client import (
        GPUEmbeddingClient,
        create_client,
    )
    ...
    pytest.skip(
        f"GPU embedding client modules not available: {e}", allow_module_level=True
    )
...
class TestGPUEmbeddingClient:
    ...
    def client(self):
        ...
        return GPUEmbeddingClient(
            service_url="http://localhost:5002", timeout=5, max_retries=1
        )
...
class TestGPUEmbeddingAdapter:
    ...
    def test_adapter_initialization(self, mock_create_client):
        ...
        adapter = GPUEmbeddingAdapter(service_url="http://localhost:5002", timeout=30)
    ...
    def test_get_text_embedding(self, mock_create_client):
        ...
        mock_client.get_models.return_value = {"embed_dim": 768}
        mock_create_client.return_value = mock_client
    ...
    def test_fallback_when_service_unavailable(self, mock_create_client):
        ...
        adapter = GPUEmbeddingAdapter(
            fallback_embedding=mock_fallback, fallback_enabled=True
        )
...
class TestGPUEmbeddingServiceReal:
    ...
    def client(self):
        ...
        return GPUEmbeddingClient(
            service_url=os.environ.get(
                "GPU_EMBEDDING_SERVICE_URL", "http://localhost:5002"
            ),
            timeout=30,
        )
    ...
    def test_real_embedding_generation(self, client):
        ...
        print(
            f"Generated embedding with {len(result['embedding'])} dimensions in {result.get('processing_time_ms', 'N/A')}ms"
        )
    ...
    def test_real_batch_embedding(self, client):
        ...
        print(
            f"Generated {result['count']} embeddings in {result.get('processing_time_ms', 'N/A')}ms"
        )
...
class TestCreateClientFactory:
    ...
    def test_create_client_with_defaults(self, tmp_path):
        ...
        plugin_json = {
            "config": {"service_url": "http://localhost:5002", "timeout": 30}
        }
        ...
        # Patch the file path
        with patch(
            "plugins.gpu_embedding.client.gpu_embedding_client.Path"
        ) as mock_path:
    ...
    def test_create_client_with_overrides(self):
        ...
        client = create_client(service_url="http://custom:9999", timeout=60)
