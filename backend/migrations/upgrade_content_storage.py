    ...
    with app.app_context():
        try:
            ...
            if dialect_name == "sqlite":
                ...
                logger.info(
                    "SQLite detected - TEXT type already supports large content"
                )
                ...
                if current_type:
                    ...
                    if "longtext" not in current_type[0].lower():
                        ...
                        db.session.execute(
                            text("ALTER TABLE documents MODIFY COLUMN content LONGTEXT")
                        )
                        db.session.commit()
                ...
                logger.info(
                    "PostgreSQL detected - TEXT type already supports large content"
                )
                ...
                if column_info:
                    logger.info(
                        f"Content column type: {column_info[0]}, max length: {column_info[1]}"
                    )
                else:
                    logger.warning("Content column not found in documents table")
            ...
            try:
                ...
                # Read it back
                retrieved_doc = (
                    db.session.query(Document).filter_by(id=test_doc.id).first()
                )
