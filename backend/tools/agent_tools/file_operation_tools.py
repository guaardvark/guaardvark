...
class ListFilesTool(BaseTool):
    ...
    def execute(
        self, directory: str, recursive: bool = False, pattern: Optional[str] = None
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if not path.is_dir():
                return ToolResult(
                    success=False, error=f"Path is not a directory: {directory}"
                )
            ...
            for item in file_list:
                ...
                if item.is_file():
                    files.append(
                        {"path": rel_path, "size": item.stat().st_size, "type": "file"}
                    )
                elif item.is_dir():
                    dirs.append({"path": rel_path, "type": "directory"})
    ...
    def execute(self, file_path: str) -> ToolResult:
        ...
        try:
            ...
            if result:
                ...
                return ToolResult(
                    success=False, error=f"Failed to process file: {file_path}"
                )

        except ImportError as e:
            logger.error(f"Enhanced file processor not available: {e}")
            return ToolResult(
                success=False, error="File processing system not available"
            )
        except Exception as e:
            logger.error(f"Error processing file {file_path}: {e}", exc_info=True)
            return ToolResult(success=False, error=f"Failed to process file: {str(e)}")
