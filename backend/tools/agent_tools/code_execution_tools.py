...
class ExecutePythonTool(BaseTool):
    ...
    name = "execute_python"
    description = (
        "Execute Python code safely in an isolated environment and return the output"
    )
    parameters = {
        "code": ToolParameter(
            name="code",
...
class ExecuteShellTool(BaseTool):
    ...
    def execute(
        self, command: str, timeout: int = 30, working_directory: str = "/tmp"
    ) -> ToolResult:
        ...
        try:
            ...
            result = execute_command(
                command, timeout, working_directory, use_shell=True
            )
