    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if result.get("success"):
                ...
                output_lines = [
                    f"MCP Servers: {len(servers)} configured, {connected} connected"
                ]
                for server in servers:
                    status = (
                        "✓ connected" if server.get("connected") else "○ not connected"
                    )
                    tools = (
                        f" ({server.get('tool_count')} tools)"
                ...
                return ToolResult(
                    success=True, output="\n".join(output_lines), metadata=result
                )
            else:
...
class MCPGetStateTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        try:
            ...
            return ToolResult(
                success=True, output="\n".join(output_lines), metadata=state
            )
