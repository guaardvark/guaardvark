...
class BrowserClickTool(BaseTool):
    ...
    parameters = {
        "url": ToolParameter(
            name="url", type="string", required=True, description="URL of the page"
        ),
...
class BrowserFillTool(BaseTool):
    ...
    name = "browser_fill"
    description = (
        "Fill a form input field with a value. Can optionally submit the form."
    )
    parameters = {
        "url": ToolParameter(
            name="url", type="string", required=True, description="URL of the page"
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        if not url or not selector or value is None:
            return ToolResult(
                success=False, error="URL, selector, and value are required"
            )
        ...
        try:
            ...
            result = _run_async(
                service.fill_form(
                    url, selector, value, submit=submit, submit_selector=submit_selector
                )
            )
...
class BrowserExtractTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            service = get_browser_service()
            result = _run_async(
                service.extract(url, selector, attribute=attribute, multiple=multiple)
            )
            ...
            if result.get("success"):
                ...
                if multiple:
                    output = (
                        f"Extracted {result.get('count', 0)} items from '{selector}'"
                    )
                else:
...
class BrowserWaitTool(BaseTool):
    ...
    name = "browser_wait"
    description = (
        "Wait for an element to be visible, hidden, attached, or detached from the DOM."
    )
    parameters = {
        "url": ToolParameter(
            name="url", type="string", required=True, description="URL of the page"
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            service = get_browser_service()
            result = _run_async(
                service.wait_for(url, selector, state=state, timeout=timeout)
            )
