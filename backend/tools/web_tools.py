...
class WebAnalysisTool(BaseTool):
        ...
        try:
            ...
            analysis = {
                ...
                "content_preview": (
                    ...
                    if len(content_result.get("content", "")) > 500
                    else content_result.get("content", "")
                ),
            ...
            return ToolResult(
                success=False, error=f"Analysis failed: {str(e)}", output={"url": url}
            )
    ...
    def _analyze_metadata(self, content_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "description_optimal": 120
            <= len(content_result.get("description", ""))
            <= 160,
        }
    ...
    def _analyze_seo(self, content_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        heading_count = (
            content.count("<h1>") + content.count("<h2>") + content.count("<h3>")
        )
        ...
        return {
            ...
            "estimated_reading_time": round(
                word_count / 200, 1
            ),  # Average reading speed
            "content_density": (
                "high" if word_count > 1000 else "medium" if word_count > 300 else "low"
            ),
        }

    def _analyze_structure(
        self, url: str, content_result: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _analyze_content(self, content_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_article = any(
            word in content_lower for word in ["article", "post", "blog", "news"]
        )
        is_product = any(
            word in content_lower for word in ["price", "buy", "cart", "product"]
        )
        is_landing = any(
            word in content_lower
            for word in ["sign up", "get started", "learn more", "contact us"]
        )
        ...
        return {
            ...
            "readability": (
                ...
                else "medium" if avg_sentence_length < 30 else "low"
            ),
        }
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not search_results or not search_results.get("success"):
                ...
                return ToolResult(
                    success=False, error=error_msg, output={"query": query}
                )
            ...
            return ToolResult(
                success=False, error=f"Search failed: {str(e)}", output={"query": query}
            )
