...
class PluginRegistry:
    ...
    def discover_plugins(self) -> List[str]:
        ...
        for item in self.plugins_dir.iterdir():
            ...
            logger.debug(
                f"Checking {item.name} for plugin.json: {plugin_json.exists()}"
            )
            ...
            if plugin_json.exists():
                try:
                    ...
                    logger.info(
                        f"✓ Discovered plugin: {metadata.id} ({metadata.name}) v{metadata.version}"
                    )
        ...
        logger.info(
            f"Plugin discovery complete: {len(discovered)} plugin(s) discovered"
        )
        if discovered:
    ...
    def get_plugins_by_type(self, plugin_type: str) -> List[PluginMetadata]:
        ...
        return [meta for meta in self._plugins.values() if meta.type == plugin_type]

    def get_plugins_by_category(self, category: str) -> List[PluginMetadata]:
        """Get all plugins in a specific category"""
        return [meta for meta in self._plugins.values() if meta.category == category]

    def get_enabled_plugins(self) -> List[PluginMetadata]:
        """Get all enabled plugins"""
        return [meta for meta in self._plugins.values() if meta.config.enabled]
    ...
    def update_plugin_config(
        self, plugin_id: str, config_updates: Dict[str, Any]
    ) -> bool:
        """
