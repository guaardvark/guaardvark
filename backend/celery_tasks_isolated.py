...
def enhanced_code_aware_indexing(file_path, document, document_id, update_progress):
    ...
    try:
        ...
        update_progress(
            75, f'Analyzing file type and content for {document["filename"]}'
        )
        ...
        update_progress(
            80,
            f'Processing content using CodeChunker strategy for {document["filename"]}',
        )
        ...
        try:
            ...
            metadata = {
                ...
                "processing_strategy": (
                    "code_preserving" if is_code_file else "standard"
                ),
                "chunking_method": (
                    "complete_file" if len(content) <= 50000 else "logical_sections"
                ),
                "processed_at": datetime.datetime.now().isoformat(),
                "indexing_method": "enhanced_code_aware",
                "preservation_mode": (
                    "complete_file"
                    if is_code_file and len(content) <= 50000
                    else "semantic_chunks"
                ),
            }
            ...
            if is_code_file:
                ...
                logger.info(
                    f"Enhanced code file indexing completed for document {document_id}: {metadata['line_count']} lines, language: {metadata['language']}"
                )
            else:
                ...
                logger.info(
                    f"Enhanced text file indexing completed for document {document_id}: {metadata['line_count']} lines"
                )
            ...
            update_progress(
                90,
                f'Adding to vector index for semantic search: {document["filename"]}',
            )
            ...
            try:
                ...
                logger.info(
                    f"Adding document {document_id} to vector index for RAG search"
                )
                ...
                vector_result = add_text_to_index(
                    content, metadata=doc_metadata, project_id=project_id_str
                )
                if vector_result:
                    logger.info(
                        f"Successfully added document {document_id} to vector index"
                    )
                    update_progress(
                        95, f'Vector indexing complete for {document["filename"]}'
                    )
                    vector_indexed = True
                else:
                ...
                logger.warning(
                    f"Could not import indexing_service (may be expected in isolated worker): {import_error}"
                )
            except Exception as vector_error:
                logger.error(
                    f"Vector indexing failed for document {document_id}: {vector_error}"
                )
            ...
            logger.info(
                f"Successfully indexed document {document_id} using enhanced code-aware method:"
            )
        ...
        logger.error(
            f"Error in enhanced code-aware indexing for document {document_id}: {e}"
        )
        # Update error status
        try:
            update_document_status(
                document_id, "ERROR", f"Enhanced indexing failed: {str(e)}"
            )
        except Exception:
            pass
        return False
        ...
        try:
            ...
            if is_code_file:
                ...
                try:
                    ...
                    session.commit()
                    logger.info(
                        f"Enhanced code file indexing completed for document {document_id}: {metadata['line_count']} lines, {metadata['char_count']} characters"
                    )
...
def index_document_task(document_id, process_id=None):
    ...
    logger.info(
        f"Starting isolated indexing task for document {document_id} with process ID: {process_id}"
    )
        ...
        if progress_system and process_id:
            try:
                ...
                if not existing_process:
                    ...
                    logger.info(
                        f"Process {process_id} not found in worker, creating new process"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Successfully created process {process_id} in worker (created_id: {created_id})"
                        )
                    except Exception as create_error:
                        logger.warning(
                            f"Could not create process via API: {create_error}, will use updates directly"
                        )
                else:
                    logger.info(f"Successfully found existing process {process_id}")
            except Exception as check_error:
                logger.warning(
                    f"Could not check process existence: {check_error}, will proceed with updates"
                )
    ...
    try:
        ...
        # Log progress
        update_progress(
            65, f'Document found: {document["filename"]}, starting indexing...'
        )
        logger.info(f'Document found: {document["filename"]}, starting indexing...')

        # Construct full file path
        upload_dir = os.environ.get(
            "GUAARDVARK_UPLOAD_DIR", os.path.join(STORAGE_DIR, "uploads")
        )
        ...
        try:
            ...
            success = enhanced_code_aware_indexing(
                full_file_path, document, document_id, update_progress
            )
        except Exception as e:
            logger.error(
                f"Enhanced indexing failed: {e}, falling back to simple indexing"
            )
            update_progress(
                75, f'Using fallback indexing due to error for {document["filename"]}'
            )
        ...
        if success:
            ...
            update_progress(
                100, f'Indexing completed for {document["filename"]}', "complete"
            )
            if progress_system:
                progress_system.complete_process(
                    process_id, f'Successfully indexed {document["filename"]}'
                )
...
def generate_bulk_csv_v2_task(
    rule_id, project_id, target_site, source_site, target_pages, process_id=None
):
...
def bulk_import_documents_task(
    job_id,
    source_path,
    target_folder,
    project_id=None,
    ...
    try:
        ...
        if target_folder:
            ...
            folder_path_for_db = (
                target_folder if target_folder.startswith("/") else f"/{target_folder}"
            )
            ...
            try:
                ...
                result = session.execute(
                    text(
                        "SELECT id, path FROM folders WHERE path = :folder_path OR name = :folder_name"
                    ),
                ...
                if folder_row:
                    ...
                    logger.info(
                        f"Found existing folder: {target_folder} (ID: {folder_id}, path: {folder_path_for_db})"
                    )
                elif not dry_run:
                    ...
                    session.commit()
                    logger.info(
                        f"Created new folder: {target_folder} (ID: {folder_id}, path: {folder_path_for_db})"
                    )
                else:
                    # Dry run - just use the path
                    logger.info(
                        f"Dry run: Would create folder: {target_folder} at {folder_path_for_db}"
                    )
            except Exception as e:
        ...
        for root, dirs, files in os.walk(source_path):
            for file in files:
                ...
                if (
                    file_ext in supported_extensions or not file_ext
        ...
        if dry_run:
            update_job_status(
                "complete",
                f"Dry run: Would import {total_files} files",
                100,
            ...
            return {
                ...
                "files": [f[2] for f in files_to_import],
            }
        ...
        try:
            for idx, (source_file, rel_path, filename) in enumerate(files_to_import):
                ...
                update_job_status(
                    "processing",
                    f"Importing {filename} ({idx+1}/{total_files})",
                    progress,
                )
                ...
                try:
                    ...
                    if folder_path_for_db:
                        ...
                        db_path = (
                            f"{folder_path_for_db}/{os.path.basename(target_file)}"
                        )
                    else:
                        # Fallback: use target_folder name without leading slash
                        db_path = os.path.join(
                            target_folder or "Imports", os.path.basename(target_file)
                        )

                    # Check if document already exists
                    result = session.execute(
                        text(
                            "SELECT id, index_status FROM documents WHERE path = :db_path"
                        ),
                        {"db_path": db_path},
                    )
                    ...
                    if existing:
                        ...
                        if reindex_missing and index_status != "INDEXED":
                            ...
                            try:
                                from backend.celery_tasks_isolated import (
                                    index_document_task,
                                )

                                index_document_task.apply_async(
                                    (doc_id,), queue="indexing"
                                )
                                indexed_count += 1
                            except Exception as e:
                                logger.warning(
                                    f"Could not queue reindexing for document {doc_id}: {e}"
                                )
                        skipped_count += 1
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Could not queue indexing for document {doc_id}: {e}"
                            )
        ...
        return {"job_id": job_id, "status": "complete", "stats": stats}
