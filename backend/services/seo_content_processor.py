...
class SEOContentProcessor:
    ...
    def _analyze_title_seo(self) -> Dict[str, Any]:
        ...
        if seo_title:
            ...
            if 50 <= len(seo_title) <= 60:
                ...
                issues.append(
                    f"SEO title length is {len(seo_title)} characters (optimal: 50-60)"
                )
    ...
    def _analyze_description_seo(self) -> Dict[str, Any]:
        ...
        if description:
            ...
            if 150 <= len(description) <= 160:
                ...
                issues.append(
                    f"Description length is {len(description)} characters (optimal: 150-160)"
                )
        ...
        if (
            focus_keywords
            and description
            and any(kw.lower() in description.lower() for kw in focus_keywords)
        ):
            score += 20
        elif focus_keywords:
    ...
    def _analyze_content_seo(self) -> Dict[str, Any]:
        ...
        if focus_keywords and content:
            ...
            if 1 <= keyword_density <= 3:
                ...
                issues.append(
                    f"Keyword density is {keyword_density:.2f}% (optimal: 1-3%)"
                )
                recommendations.append(
    ...
    def _analyze_schema_seo(self) -> Dict[str, Any]:
        ...
        if schema_markup:
            ...
            if isinstance(schema_markup, list) and len(schema_markup) > 1:
                score = 100
                recommendations.append(
                    "Multiple schema types detected - ensure they're compatible"
                )
        else:
    ...
    def _analyze_performance_seo(self) -> Dict[str, Any]:
        ...
        if desktop_score is not None:
            if desktop_score >= 90:
                ...
                issues.append(
                    f"Desktop PageSpeed score is {desktop_score} (target: 90+)"
                )
        ...
        if analytics_data:
            ...
            if analytics_data.get("avg_position", 0) > 0:
                ...
                if position <= 10:
                    ...
                    recommendations.append(
                        f"Average position is {position} - improve ranking"
                    )
        else:
    ...
    def _update_score_history(self, score: int) -> None:
        ...
        history.append({"date": datetime.now().isoformat(), "score": score})
