...
class BaseTool:
    ...
    parameters: Dict[str, ToolParameter] = (
        None  # Set in subclass, don't use mutable default
    )
...
class ToolRegistry:
        ...
        if format == "xml":
            ...
            for tool in self.tools.values():
                ...
                for pname, pinfo in schema["parameters"].items():
                    ...
                    params_desc.append(
                        f'    "{pname}": {pinfo["type"]}{req} - {pinfo["description"]}'
                    )
    ...
    def execute_tool(
        self, tool_name: str, agent_context: Optional[Dict[str, Any]] = None, **kwargs
    ) -> ToolResult:
        """
        ...
        if not tool.can_execute(**kwargs):
            ...
            expected_params = [
                name for name, param in tool.parameters.items() if param.required
            ]
