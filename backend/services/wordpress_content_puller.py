...
class WordPressContentPuller:
    ...
    def __init__(self, site_id: int, job_id: Optional[str] = None):
        ...
        connection_type = (
            getattr(self.wp_site, "connection_type", "llamanator") or "llamanator"
        )

        if connection_type == "llamanator":
            # Use LLAMANATOR2 plugin API
            self.wp_service = LlamanatorAPIService(
                site_url=self.wp_site.url, api_key=self.wp_site.api_key
            )
        else:
            # Use direct WordPress REST API (legacy)
            if not self.wp_site.username:
                raise ValueError(
                    "Username is required for direct WordPress REST API connection"
                )
        ...
        try:
            ...
            while True:
                result = self.wp_service.get_posts(
                    post_type=post_type, per_page=per_page, page=page, **pull_filters
                )

                if not result["success"]:
                    self.progress_system.error_process(
                        self.job_id, result.get("error", "Unknown error")
                    )
                    return {
                        "success": False,
                ...
                if "posts" in result:
                    ...
                    if total_pages is None:
                        total_pages = result.get(
                            "pages", result.get("pagination", {}).get("total_pages", 1)
                        )
                        total_items = result.get(
                            "total",
                            result.get("pagination", {}).get("total_items", len(posts)),
                        )
            ...
            self._update_progress(
                95, f"Successfully pulled {len(all_posts)} posts, finalizing..."
            )
            ...
            for idx, post in enumerate(all_posts):
                try:
                    ...
                    existing_page = (
                        db.session.query(WordPressPage)
                        .filter_by(
                            wordpress_site_id=self.site_id, wordpress_post_id=post_id
                        )
                        .first()
                    )
                    ...
                    if existing_page:
                        ...
                        existing_page.categories = json.dumps(
                            categories if isinstance(categories, list) else []
                        )
                        existing_page.tags = json.dumps(
                    ...
                    if (idx + 1) % 100 == 0:
                        ...
                        try:
                            db.session.commit()
                            logger.info(
                                f"Periodic commit successful: saved {saved_count}, updated {updated_count} so far"
                            )
                        except Exception as e:
                            logger.error(
                                f"Periodic commit failed at post {idx + 1}: {e}",
                                exc_info=True,
                            )
                            db.session.rollback()
                    ...
                    logger.error(
                        f"Error saving post {post.get('id')}: {e}", exc_info=True
                    )
                    skipped_count += 1
                    continue
            ...
            try:
                db.session.commit()
                logger.info(
                    f"Final database commit successful: Saved {saved_count} new posts and updated {updated_count} existing posts. Skipped {skipped_count}."
                )
            except Exception as commit_error:
                db.session.rollback()
                logger.error(
                    f"Final database commit failed: {commit_error}", exc_info=True
                )
                self.progress_system.error_process(
                    self.job_id, f"Database commit failed: {str(commit_error)}"
                )
                return {
                    "success": False,
            ...
            logger.info(
                f"Successfully saved {saved_count} new posts and updated {updated_count} existing posts. Skipped {skipped_count}."
            )
    ...
    def pull_single_page(
        self, post_id: int, post_type: str = "post", update_existing: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not post:
                return {"success": False, "error": "Post data not found in response"}

            # Check if page already exists
            existing_page = (
                db.session.query(WordPressPage)
                .filter_by(wordpress_site_id=self.site_id, wordpress_post_id=post_id)
                .first()
            )
            ...
            if post.get("categories"):
                ...
                if cat_result["success"]:
                    ...
                    # LlamanatorAPIService returns: {"success": True, "data": [...]}
                    cat_list = cat_result.get("categories") or cat_result.get(
                        "data", []
                    )
                    cat_map = {cat["id"]: cat["name"] for cat in cat_list}
                    categories = [
                        cat_map.get(cid, "") for cid in post.get("categories", [])
                    ]
            ...
            if post.get("author"):
                author_name = (
                    f"Author {post['author']}"  # Would need user endpoint for full name
                )
            ...
            logger.info(
                f"{action.capitalize()} WordPress page: {post_id} from site {self.site_id} (LLM010.3 ID: {wp_page.id})"
            )

            return {"success": True, "data": wp_page.to_dict(), "action": action}
    ...
    def pull_bulk_pages(
        self, post_ids: List[int], post_type: str = "post"
    ) -> Dict[str, Any]:
        """
    ...
    def pull_from_sitemap(self, extract_post_ids: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if urls is None:
                return {"success": False, "error": "Could not find or parse sitemap"}
            ...
            self._update_progress(
                100, f"Successfully parsed sitemap with {len(urls)} URLs"
            )
    ...
    def _extract_and_store_seo_data(
        self, wp_page: WordPressPage, post_data: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if isinstance(focus_keywords, str):
                ...
                focus_keywords = [
                    k.strip() for k in focus_keywords.split(",") if k.strip()
                ]
            wp_page.focus_keywords = (
                json.dumps(focus_keywords) if focus_keywords else None
            )
            ...
            try:
                seo_analysis = self.wp_service.get_seo_analysis(
                    wp_page.wordpress_post_id
                )
                ...
                logger.debug(
                    f"Failed to fetch SEO analysis for post {wp_page.wordpress_post_id}: {e}"
                )

            # Fetch and store analytics data
            try:
                analytics = self.wp_service.get_seo_analytics(
                    wp_page.wordpress_post_id, days=30
                )
                ...
                logger.debug(
                    f"Failed to fetch analytics for post {wp_page.wordpress_post_id}: {e}"
                )

            # Fetch and store PageSpeed data
            try:
                pagespeed = self.wp_service.get_pagespeed_data(
                    wp_page.wordpress_post_id
                )
                if pagespeed.get("success"):
                    pagespeed_data = pagespeed.get("data", {})
                    wp_page.pagespeed_score_mobile = pagespeed_data.get(
                        "mobile", {}
                    ).get("score")
                    wp_page.pagespeed_score_desktop = pagespeed_data.get(
                        "desktop", {}
                    ).get("score")
                ...
                logger.debug(
                    f"Failed to fetch PageSpeed for post {wp_page.wordpress_post_id}: {e}"
                )

        except Exception as e:
            logger.warning(
                f"Error extracting SEO data for post {wp_page.wordpress_post_id}: {e}",
                exc_info=True,
            )
    ...
    def get_pull_status(self) -> Dict[str, Any]:
        ...
        try:
            total_pages = (
                db.session.query(WordPressPage)
                .filter_by(wordpress_site_id=self.site_id)
                .count()
            )

            pulled_pages = (
                db.session.query(WordPressPage)
                .filter_by(wordpress_site_id=self.site_id, pull_status="pulled")
                .count()
            )

            error_pages = (
                db.session.query(WordPressPage)
                .filter_by(wordpress_site_id=self.site_id, pull_status="error")
                .count()
            )

            pending_pages = (
                db.session.query(WordPressPage)
                .filter_by(wordpress_site_id=self.site_id, pull_status="pending")
                .count()
            )
            ...
            return {
                ...
                "errors": error_pages,
                "last_pull_at": (
                    self.wp_site.last_pull_at.isoformat()
                    if self.wp_site.last_pull_at
                    else None
                ),
            }
        except Exception as e:
