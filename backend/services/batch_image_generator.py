...
try:
    ...
    from backend.utils.system_coordinator import (
        SystemCoordinator,
        ProcessType as CoordProcessType,
        ResourceType,
    )
...
class BatchImageGenerator:
    ...
    def __init__(self):
        ...
        self.progress_system = (
            UnifiedProgressSystem() if offline_gen_available else None
        )
        ...
        logger.info(
            f"BatchImageGenerator initialized - Service available: {self.service_available}"
        )
    ...
    def _parse_csv_prompts(self, csv_content: str) -> List[BatchPrompt]:
        ...
        try:
            ...
            for i, row in enumerate(csv_reader):
                ...
                try:
                    width = (
                        int(row.get("width", 512))
                        if row.get("width", "").strip()
                        else 512
                    )
                    height = (
                        int(row.get("height", 512))
                        if row.get("height", "").strip()
                        else 512
                    )
                    steps = (
                        int(row.get("steps", 20))
                        if row.get("steps", "").strip()
                        else 20
                    )
                    guidance = (
                        float(row.get("guidance", 7.5))
                        if row.get("guidance", "").strip()
                        else 7.5
                    )
                    seed = (
                        int(row["seed"])
                        if row.get("seed") and row["seed"].strip()
                        else None
                    )
                    ...
                    logger.warning(
                        f"Row {i+1} has invalid numeric values, using defaults: {e}"
                    )
                    width = 512
                    height = 512
                    steps = 20
                ...
                prompt = BatchPrompt(
                    ...
                    prompt=prompt_text,
                    negative_prompt=(
                        row.get("negative_prompt", "").strip()
                        if row.get("negative_prompt")
                        else ""
                    ),
                    style=(
                        row.get("style", "realistic").strip()
                        if row.get("style")
                        else "realistic"
                    ),
                    width=width,
            ...
            if not prompts:
                raise ValueError(
                    "No valid prompts found in CSV. Ensure CSV has a 'prompt' column with non-empty values."
                )
    ...
    def _generate_single_image(
        ...
        try:
            ...
            face_restoration_weight = getattr(
                batch_status, "face_restoration_weight", 0.5
            )
            ...
            if result.success and result.image_path:
                ...
                if (
                    batch_status
                    and hasattr(batch_status, "generate_thumbnails")
                    and batch_status.generate_thumbnails
                ):
                    thumbnail_path = self._create_thumbnail(
                        str(target_path), output_dir / "thumbnails"
                    )
                ...
                logger.error(
                    f"Image generation failed for prompt {prompt.id}: {error_msg}"
                )
            ...
            logger.error(
                f"Exception during image generation for prompt {prompt.id}: {e}"
            )
            return BatchImageResult(
                prompt_id=prompt.id,
                success=False,
    ...
    def _save_batch_metadata(
        self, batch_status: BatchGenerationStatus, output_dir: Path
    ):
        try:
            metadata = {
                ...
                "start_time": (
                    batch_status.start_time.isoformat()
                    if batch_status.start_time
                    else None
                ),
                "end_time": (
                    batch_status.end_time.isoformat() if batch_status.end_time else None
                ),
                "generation_summary": {
                    "total_generation_time": sum(
                        r.generation_time for r in batch_status.results
                    ),
    ...
    def start_batch_generation(self, request: BatchImageRequest) -> str:
        ...
        def run_batch():
            try:
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    while prompt_index < len(request.prompts) or pending_futures:
                        if batch_status.status == "cancelled":
                            logger.info(
                                f"Batch {batch_id} cancelled, stopping new submissions"
                            )
                        ...
                        while (
                            ...
                            and len(pending_futures) < max_workers
                        ):
                            if batch_status.status == "cancelled":
                                break
                ...
                batch_status.status = (
                    "completed" if batch_status.status != "cancelled" else "cancelled"
                )
                ...
                if (
                    ...
                    and batch_status.completed_images > 0
                ):
                    try:
                        from flask import current_app
                        from backend.services.image_registration_service import (
                        ...
                        with app.app_context():
                            batch_name = (
                                getattr(request, "batch_name", None) or batch_id
                            )
                            register_batch_images(
                                batch_id=batch_id,
                            ...
                            logger.info(
                                f"Registered batch {batch_id} images into Documents system"
                            )
                    except Exception as reg_err:
                        logger.error(
                            f"Failed to register batch images into Documents system: {reg_err}"
                        )
        ...
        logger.info(
            f"Started batch generation {batch_id} with {len(request.prompts)} prompts"
        )
        return batch_id
    ...
    def start_blueprint_batch(self, csv_content: str) -> str:
        ...
        try:
            ...
            total = sum(
                1
                for row in reader
                if (row.get("city") or row.get("City") or row.get("name"))
            )
        except Exception:
            total = 0
    ...
    def _run_blueprint_batch(
        ...
        for row in csv_input:
            ...
            count_raw = (
                row.get("count") or row.get("patents") or row.get("value") or "100"
            )
            try:
            ...
            state = (
                state_raw.strip().lower() if state_raw and state_raw.strip() else "oh"
            )
            ...
            try:
                ...
                if style == "tech":
                    hero_color = colors["nodes"][
                        sum(ord(c) for c in city) % len(colors["nodes"])
                    ]
                    for i in range(density):
                        ...
                        if i > 0 and i % 3 == 0:
                            ...
                            draw.line(
                                [x, y, prev[0], prev[1]], fill=colors["lines"], width=1
                            )
                    for x, y in nodes:
                    ...
                    for i, (x1, y1) in enumerate(nodes):
                        ...
                        check_indices = random.sample(
                            range(len(nodes)), min(20, len(nodes))
                        )
                    ...
                    for i in range(density):
                        ...
                        node_color = (
                            colors["nodes"][1] if is_crisis else colors["nodes"][0]
                        )
                    ...
                    for i in range(density):
                        ...
                        if i > 0 and i % 2 == 0:
                            ...
                            mid_y = gy
                            draw.line(
                                [gx, gy, mid_x, mid_y], fill=colors["lines"], width=1
                            )
                            draw.line(
                                [mid_x, mid_y, prev[0], prev[1]],
                                fill=colors["lines"],
                                width=1,
                            )

                        draw.rectangle(
                            [gx - 2, gy - 2, gx + 2, gy + 2], fill=colors["nodes"][0]
                        )
                    ...
                    for ty in trace_y_lanes:
                        ...
                        draw.line(
                            [40, ty + jitter, width - 40, ty + jitter],
                            fill=colors["lines"],
                            width=1,
                        )
                    ...
                    for i in range(density):
                        ...
                        if i > 0 and i % 4 == 0:
                            ...
                            draw.line(
                                [x, y, x, target_lane], fill=colors["lines"], width=1
                            )
                    ...
                    for i, (mx, y) in enumerate(right_nodes):
                        ...
                        if i > 0 and i % 3 == 0:
                            ...
                            draw.line(
                                [mx, y, prev[0], prev[1]], fill=colors["lines"], width=1
                            )
                        ...
                        while x < width - 40:
                            if random.random() < 0.08:
                                ...
                                points.extend(
                                    [
                                        (x, base_y),
                                        (x + 4, base_y - spike_h * direction),
                                        (x + 8, base_y + spike_h * direction // 3),
                                        (x + 12, base_y),
                                    ]
                                )
                                x += 16
                    ...
                    for px in range(post_spacing, width - 40, post_spacing):
                        draw.line(
                            [px, 40, px, height - 40], fill=colors["nodes"][2], width=3
                        )
        ...
        metadata = {
            ...
            "failed_images": batch_status.failed_images,
            "start_time": (
                batch_status.start_time.isoformat() if batch_status.start_time else None
            ),
            "end_time": (
                batch_status.end_time.isoformat() if batch_status.end_time else None
            ),
        ...
        logger.info(
            f"Blueprint batch {batch_id} completed: {batch_status.completed_images}/{batch_status.total_images}"
        )
    ...
    def list_all_batches(self) -> List[BatchGenerationStatus]:
        ...
        all_batches.sort(
            key=lambda b: b.start_time if b.start_time else datetime.min, reverse=True
        )
    ...
    def create_batch_from_prompts(
        self, prompt_list: List[str], **kwargs
    ) -> BatchImageRequest:
        prompt_params = [
            "model",
            "style",
    ...
    def get_service_status(self) -> Dict[str, Any]:
        with self.batch_lock:
            active_batches = len(
                [b for b in self.active_batches.values() if b.status == "running"]
            )
            ...
            try:
                if hasattr(self.image_generator, "get_service_status"):
                    ...
                    if isinstance(status, dict):
                        ...
                        logger.warning(
                            f"Image generator status returned non-dict type: {type(status)}"
                        )
                        image_generator_status = {
                            "service_available": hasattr(
                                self.image_generator, "service_available"
                            )
                            and self.image_generator.service_available,
                            "error": f"Status format not serializable: {type(status)}",
                        }
                else:
                    logger.warning(
                        "Image generator does not have get_service_status method"
                    )
                    image_generator_status = {
                        "service_available": hasattr(
                            self.image_generator, "service_available"
                ...
                image_generator_status = {"service_available": False, "error": str(e)}
