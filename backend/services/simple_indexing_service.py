...
try:
    ...
    logger.critical(
        f"Failed to import local dependencies for simple_indexing_service: {e}.",
        exc_info=True,
    )
    DBDocument = None
    db = None
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            update_document_status(
                db_document.id, "ERROR", f"File not found: {file_path}"
            )
            return False
        ...
        with _index_lock:
            ...
            _document_store[doc_key] = {
                ...
                "metadata": {
                    ...
                    "project_id": (
                        str(db_document.project_id) if db_document.project_id else None
                    ),
        ...
        logger.info(
            f"Successfully indexed document {db_document.id} with {len(chunks)} chunks"
        )
        return True
...
def get_index_stats() -> Dict[str, Any]:
    ...
    with _index_lock:
        return {
            ...
            "total_chunks": sum(
                doc["metadata"]["chunk_count"] for doc in _document_store.values()
            ),
            "total_content_length": sum(
                doc["metadata"]["total_length"] for doc in _document_store.values()
            ),
            "documents": [
                {
                    "id": doc["id"],
