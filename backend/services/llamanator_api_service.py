...
class LlamanatorAPIService:
    ...
    def test_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        if result["success"]:
            ...
            if error_code == "invalid_api_key" or "Invalid API key" in error_msg:
                return (
                    False,
                    "Invalid API key. Please check the API key in LLAMANATOR2 plugin settings and ensure it matches exactly.",
                )
            elif error_code == "missing_api_key" or "API key is required" in error_msg:
                return (
                    False,
                    "API key is required. Please provide the LLAMANATOR2 API key.",
                )
            elif error_code == "api_key_not_configured":
                return (
                    False,
                    "API key not configured on WordPress site. Please generate one in LLAMANATOR2 plugin settings (Settings → LLAMANATOR2 → LLM010.3 Integration).",
                )
            elif "Connection error" in error_msg:
                return (
                    False,
                    f"Cannot connect to WordPress site: {error_msg}. Please check the site URL and ensure it's accessible.",
                )
    ...
    def get_post(self, post_id: int, post_type: str = "post") -> Dict[str, Any]:
        ...
        if result["success"]:
            return {"success": True, "data": self._transform_post_data(result["data"])}
    ...
    def get_bulk_posts(
        ...
        """
        data = {"post_ids": post_ids, "post_type": post_type}
    ...
    def update_post(
        self, post_id: int, post_type: str = "post", data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        Update post/page
    ...
    def get_bulk_seo_analytics(
        self, post_ids: List[int], days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        """
        data = {"post_ids": post_ids, "days": days}
    ...
    def _transform_post_data(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transformed = {
            ...
            "content": {
                "rendered": post_data.get("content", ""),
                "raw": post_data.get("content", ""),
            },
            "excerpt": {
                "rendered": post_data.get("excerpt", ""),
                "raw": post_data.get("excerpt", ""),
            },
