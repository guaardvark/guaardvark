...
class WordPressAPIService:
    ...
    def test_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            response = requests.get(
                f"{self.base_api_url}/", headers=self.headers, timeout=self.timeout
            )
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"Connection failed with status {response.status_code}: {response.text[:200]}",
                )
    ...
    def update_post(
        self, post_id: int, post_type: str = "post", **data
    ) -> Dict[str, Any]:
        """
    ...
    def update_post_meta(
        self, post_id: int, meta_key: str, meta_value: Any, post_type: str = "post"
    ) -> Dict[str, Any]:
        """
