...
class SEOAnalyticsService:
    ...
    def sync_analytics_for_site(self, site_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        if connection_type != "llamanator":
            return {
                "success": False,
                "error": "Analytics sync only available for LLAMANATOR2 connections",
            }
        ...
        try:
            ...
            pages = (
                ...
                .filter_by(wordpress_site_id=site_id, pull_status="pulled")
                .all()
            )
            ...
            for page in pages:
                try:
                    # Sync analytics
                    analytics = wp_service.get_seo_analytics(
                        page.wordpress_post_id, days=days
                    )
            ...
            logger.error(
                f"Error syncing analytics for site {site_id}: {e}", exc_info=True
            )
            db.session.rollback()
    ...
    def sync_pagespeed_for_site(self, site_id: int) -> Dict[str, Any]:
        ...
        if connection_type != "llamanator":
            return {
                "success": False,
                "error": "PageSpeed sync only available for LLAMANATOR2 connections",
            }
        ...
        try:
            ...
            for page in pages:
                try:
                    ...
                    if pagespeed.get("success"):
                        ...
                        page.pagespeed_score_mobile = pagespeed_data.get(
                            "mobile", {}
                        ).get("score")
                        page.pagespeed_score_desktop = pagespeed_data.get(
                            "desktop", {}
                        ).get("score")
            ...
            logger.error(
                f"Error syncing PageSpeed for site {site_id}: {e}", exc_info=True
            )
            db.session.rollback()
            return {"success": False, "error": str(e)}
    ...
    def get_site_performance_report(
        ...
        if pages_with_analytics > 0:
            avg_position = (
                avg_position / pages_with_analytics if pages_with_analytics > 0 else 0
            )
            avg_ctr = avg_ctr / pages_with_analytics if pages_with_analytics > 0 else 0

        if pages_with_pagespeed > 0:
            avg_mobile_score = (
                avg_mobile_score / pages_with_pagespeed
                if pages_with_pagespeed > 0
                else 0
            )
        ...
        return {
            ...
            "performance": {
                "avg_mobile_score": (
                    round(avg_mobile_score, 0) if avg_mobile_score > 0 else None
                ),
                "avg_desktop_score": (
                    round(avg_desktop_score, 0) if avg_desktop_score > 0 else None
                ),
            },
            "seo_scores": {
    ...
    def get_top_performing_pages(
        self, site_id: int, metric: str = "clicks", limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        for page in pages:
            data = {
                ...
                "slug": page.slug,
                "permalink": (
                    f"{page.wordpress_site.url}/{page.slug}" if page.slug else None
                ),
                "metric_value": 0,
            }
    ...
    def get_keyword_performance(self, site_id: int, keyword: str) -> Dict[str, Any]:
        ...
        # Aggregate metrics
        total_clicks = sum(
            p.get("analytics", {}).get("clicks", 0) for p in matching_pages
        )
        total_impressions = sum(
            p.get("analytics", {}).get("impressions", 0) for p in matching_pages
        )
        avg_position = sum(
            p.get("analytics", {}).get("avg_position", 0) for p in matching_pages
        )
    ...
    def generate_seo_report(
        self, site_id: Optional[int] = None, client_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_performance_summary(
        self, pages: List[WordPressPage]
    ) -> Dict[str, Any]:
        """Calculate overall performance summary"""
        seo_scores = [p.seo_score for p in pages if p.seo_score is not None]
        mobile_scores = [
            p.pagespeed_score_mobile
            for p in pages
            if p.pagespeed_score_mobile is not None
        ]
        desktop_scores = [
            p.pagespeed_score_desktop
            for p in pages
            if p.pagespeed_score_desktop is not None
        ]
        ...
        return {
            "avg_seo_score": (
                round(sum(seo_scores) / len(seo_scores), 1) if seo_scores else None
            ),
            "avg_mobile_pagespeed": (
                round(sum(mobile_scores) / len(mobile_scores), 0)
                if mobile_scores
                else None
            ),
            "avg_desktop_pagespeed": (
                round(sum(desktop_scores) / len(desktop_scores), 0)
                if desktop_scores
                else None
            ),
            "total_clicks": total_clicks,
    ...
    def _get_top_pages_summary(
        self, pages: List[WordPressPage], limit: int = 5
    ) -> Dict[str, Any]:
        ...
        return {
            "top_by_seo_score": [
                {"title": p.title, "score": p.seo_score, "id": p.id} for p in top_seo
            ]
        }
    ...
    def _identify_improvement_opportunities(
        ...
        for page in pages:
            ...
            if (
                page.pagespeed_score_mobile is not None
                and page.pagespeed_score_mobile < 70
            ):
            ...
            if issues:
                opportunities.append(
                    {"page_id": page.id, "title": page.title, "issues": issues}
                )
    ...
    def _generate_report_recommendations(self, pages: List[WordPressPage]) -> List[str]:
        ...
        pages_with_low_score = sum(
            1 for p in pages if p.seo_score is not None and p.seo_score < 70
        )
        pages_without_pagespeed = sum(
            1 for p in pages if p.pagespeed_score_mobile is None
        )

        if pages_without_seo > 0:
            recommendations.append(
                f"{pages_without_seo} pages missing SEO metadata - add meta titles and descriptions"
            )

        if pages_with_low_score > 0:
            recommendations.append(
                f"{pages_with_low_score} pages have SEO scores below 70 - review and optimize"
            )

        if pages_without_pagespeed > 0:
            recommendations.append(
                f"{pages_without_pagespeed} pages missing PageSpeed data - run performance analysis"
            )
