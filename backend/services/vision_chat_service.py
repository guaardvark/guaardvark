...
class VisionChatService:
    ...
    def __init__(self):
        ...
        self.fallback_vision_models = (
            []
        )  # Populated dynamically by _get_available_models()
    ...
    def analyze_image_for_chat(
        self, image_data: bytes, prompt: str = "", analysis_type: str = "describe"
    ) -> ImageAnalysisResult:
        """
        ...
        result = ImageAnalysisResult(success=False, objects_detected=[], error=None)
        ...
        try:
            ...
            if analysis_type == "describe":
                analysis_prompt = (
                    prompt or """Please describe this image in detail. Include:
...
Provide a comprehensive but concise description suitable for a chat conversation."""
                ...
                analysis_prompt = (
                    prompt
                    or """Extract and transcribe all text visible in this image. Include:
...
If no text is visible, respond with 'No text detected in image'."""
                ...
                analysis_prompt = (
                    prompt
                    or """Identify and list all objects, people, and elements visible in this image. Provide:
...
Format as a clear, organized list."""
                ...
                analysis_prompt = (
                    prompt or "Please analyze this image and provide relevant insights."
                )
            ...
            try:
                ...
                if analysis_result:
                    ...
                    result.confidence = (
                        0.85  # Default confidence for successful analysis
                    )
                ...
                if image_analysis_available:
                    ...
                    extraction_result = image_extractor.extract_text_from_image(
                        image_path
                    )

                    if extraction_result.get("success"):
                        result.success = True
                        result.extracted_text = extraction_result.get(
                            "text_content", ""
                        )
                        result.description = (
                            f"Image processed with OCR. Text content: {result.extracted_text}"
                            if result.extracted_text
                            else "Image processed - no text content detected"
                        )
                        result.model_used = extraction_result.get(
                            "model_used", "image_content_service"
                        )
                        result.confidence = extraction_result.get("confidence", 0.0)
                    else:
                        result.error = extraction_result.get(
    ...
    def generate_image_for_chat(
        self, prompt: str, style: str = "realistic", size: tuple = (512, 512)
        ...
        result = ImageGenerationResult(
            success=False, prompt_used=prompt, image_size=size, error=None
        )
        ...
        try:
            ...
            try:
                ...
                if response.get("response"):
                    ...
                    image_data = response.get(
                        "image_data"
                    )  # This would depend on actual API
    ...
    def process_image_paste(
        self, image_data: bytes, user_message: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        if user_message:
            ...
            if any(
                word in message_lower for word in ["text", "read", "ocr", "transcribe"]
            ):
                analysis_type = "ocr"
            elif any(
                word in message_lower for word in ["object", "identify", "what", "list"]
            ):
                analysis_type = "objects"
            elif any(
                word in message_lower
                for word in ["analyze", "explain", "tell me about"]
            ):
        ...
        analysis_result = self.analyze_image_for_chat(
            image_data, prompt=custom_prompt, analysis_type=analysis_type
        )
        ...
        if analysis_result.success:
            ...
            if analysis_type == "ocr" and analysis_result.extracted_text:
                ...
                chat_response = (
                    f"I can see this image shows:\n\n{analysis_result.description}"
                )
            else:
                chat_response = (
                    "I've processed the image, but couldn't extract specific details."
                )
            ...
            chat_response = (
                f"I wasn't able to analyze this image properly. Error: {error_msg}"
            )

            response_data.update({"chat_response": chat_response, "error": error_msg})
...
def analyze_chat_image(
    image_data: bytes, prompt: str = "", analysis_type: str = "describe"
) -> ImageAnalysisResult:
...
def generate_chat_image(
    prompt: str, style: str = "realistic", size: tuple = (512, 512)
) -> ImageGenerationResult:
