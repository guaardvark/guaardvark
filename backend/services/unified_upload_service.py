...
class UnifiedUploadService:
    ...
    def upload_file(
        ...
        # Detect file types
        is_code = _is_code_file(original_filename)
        is_text = _is_text_file(original_filename)
        file_ext = (
        ...
        if auto_index and progress_system:
            ...
            if progress_system and job_id:
                progress_system.update_process(
                    job_id, 10, f"Uploading {original_filename}"
                )
        ...
        if progress_system and job_id:
            progress_system.update_process(job_id, 20, f"Uploading {original_filename}")
            ...
            try:
                ...
                if file_content:
                    ...
                    if len(file_content) > max_content_size:
                        ...
                        logger.warning(
                            f"File {original_filename} truncated for storage"
                        )
                    else:
                        document.content = file_content
                        document.index_status = "STORED"
                        logger.info(
                            f"Stored complete content for {'code' if is_code else 'text'} file: {original_filename}"
                        )
                else:
                    document.index_status = "STORAGE_FAILED"
                    document.error_message = "Failed to read file content"
                    logger.warning(f"Failed to store content for {original_filename}")
            except Exception as e:
                document.index_status = "STORAGE_FAILED"
                document.error_message = f"Content storage error: {str(e)}"
                logger.error(f"Error storing content for {original_filename}: {e}")

        # Commit document
        db.session.commit()
        logger.info(
            f"Document {document.id} ({original_filename}) successfully committed to database"
        )
        ...
        if progress_system and job_id:
            progress_system.update_process(
                job_id,
                50,
                f"Upload complete: {original_filename}, starting indexing...",
            )
        ...
        if auto_index:
            try:
                ...
                logger.info(
                    f"Submitted indexing task for document {document.id}: {task.id}"
                )
            except Exception as index_err:
                logger.error(
                    f"Failed to trigger indexing for document {document.id}: {index_err}"
                )
                # Don't fail the upload if indexing fails - user can manually reindex
                if progress_system and job_id:
                    progress_system.error_process(
                        job_id, f"Indexing trigger failed: {index_err}"
                    )
