    ...
    def upload_file(
        ...
        if auto_index and progress_system:
            job_id = progress_system.create_process(
                ...
                {"filename": original_filename, "size": total_size, "document_id": None}
            )
            if progress_system and job_id:
                progress_system.update_process(job_id, 10, f"Uploading {original_filename}")
            ...
            try:
                ...
                if file_content:
                    ...
                    if len(file_content) > max_content_size:
                        ...
                        logger.warning(f"File {original_filename} truncated for storage")
                    else:
                        document.content = file_content
                        document.index_status = "STORED"
                        logger.info(f"Stored complete content for {'code' if is_code else 'text'} file: {original_filename}")
                else:
        ...
        # Commit document
        db.session.commit()
        logger.info(f"Document {document.id} ({original_filename}) successfully committed to database")
        
        # Update progress
        if progress_system and job_id:
            progress_system.update_process(job_id, 50, f"Upload complete: {original_filename}, starting indexing...")
