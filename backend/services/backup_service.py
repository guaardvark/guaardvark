...
``llamaX1_backup.json`` manifest file and any referenced files.
"""
...
def create_data_backup(components: List[str] | None = None, name: str | None = None) -> str:
    ...
    try:
        ...
        with TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            json_path = tmp / "llamaX1_backup.json"
...
def create_full_backup(name: str | None = None) -> str:
    ...
    try:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            install_instructions = f"""# Guaardvark Full System Backup - Installation Instructions
...
1. **Extract the backup:**
   ...
   cd llamax1_full_system_backup_{timestamp}
   ```
...
- The backup includes all application data, source code, and configuration files
- The system will automatically install Python and Node.js dependencies on first run
- Database will be automatically migrated if needed
- All uploaded files and user data are preserved

## Troubleshooting

- If you encounter permission issues, run: `chmod +x *.sh`
- If Python dependencies fail, run: `cd backend && python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt`
- If Node.js dependencies fail, run: `cd frontend && npm install`
...
def create_code_release(name: str | None = None) -> str:
    ...
    try:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Write metadata JSON
            meta_path = tmp / "llamaX1_backup.json"
...
"""
            ...
            # Create installation instructions
            install_instructions = f"""# Guaardvark Code-Only Backup - Installation Instructions

## Backup Information
- **Backup Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
- **Version:** 1.0
- **Type:** Code Release (No Data)
...
1. **Extract the backup:**
   ```bash
   unzip {backup_name}.zip
   cd llamax1_code_only_backup_{timestamp}
   ```
...
1. Use a separate Data Backup to restore your database and files
    ...
    try:
        ...
        with ZipFile(zip_file, "r") as zf:
            ...
            for member in zf.namelist():
                if member.endswith("/") or member == "llamaX1_backup.json":
                    continue
