...
class FaceRestorationService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"FaceRestorationService initialized - Device: {self._device}, Available: {self.service_available}"
        )
    ...
    def _load_gfpgan(self) -> bool:
        ...
        try:
            ...
            if not model_path_esrgan.exists():
                logger.info(
                    "RealESRGAN model not found, GFPGAN will download it automatically"
                )
            ...
            model_path_gfpgan = (
                self.models_dir / f"GFPGANv{self.gfpgan_model_version}.pth"
            )
    ...
    def restore_face(
        ...
        try:
            ...
            logger.info(
                f"Face restoration complete: {metadata['faces_detected']} faces restored"
            )
    ...
    def restore_face_from_pil(
        self, image: Image.Image, weight: float = 0.5
    ...
    def batch_restore_faces(
        self, image_paths: list, output_dir: Optional[str] = None, weight: float = 0.5
    ) -> Dict[str, Any]:
        results = {
        ...
        for image_path in image_paths:
            ...
            success, restored_path, metadata = self.restore_face(
                image_path=image_path, output_path=output_path, weight=weight
            )
        ...
        logger.info(
            f"Batch restoration complete: {results['successful']}/{results['total']} successful"
        )
        return results
