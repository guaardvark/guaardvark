...
def discover(search_root=None):
    ...
    for root, dirs, files in os.walk(search_root):
        if any(
            x in root
            for x in [".cache", ".local", "node_modules", "venv", ".git", ".npm"]
        ):
            dirs[:] = []
            continue

        if "start.sh" in files:
            if os.path.isdir(os.path.join(root, "backend")) and os.path.isdir(
...
if __name__ == "__main__":
    ...
    print(
        f"Discovery complete. Found {len(found_paths)} environments, {count} new ones registered."
    )
