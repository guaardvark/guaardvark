...
from discord_bot.commands.chat import ChatCog, split_message
...
class TestAskCommand:
    ...
    async def test_ask_returns_response(
        self, chat_cog, mock_interaction, mock_api_client
    ):
        ...
        assert "Hello! I'm Guaardvark." in call_kwargs.kwargs.get(
            "content", ""
        ) or "Hello! I'm Guaardvark." in (
            call_kwargs.args[0] if call_kwargs.args else ""
        )

    @pytest.mark.asyncio
    async def test_ask_uses_user_session_id(
        self, chat_cog, mock_interaction, mock_api_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_ask_handles_api_error(
        self, chat_cog, mock_interaction, mock_api_client
    ):
        ...
        assert (
            "backend may be offline" in call_kwargs.get("content", "").lower()
            or "failed" in call_kwargs.get("content", "").lower()
        )

    @pytest.mark.asyncio
    async def test_ask_sanitizes_input(
        self, chat_cog, mock_interaction, mock_api_client
    ):
        """Mentions and code blocks should be stripped before sending to API."""
        await chat_cog._handle_ask(
            mock_interaction, "<@!12345> tell me about ```code```"
        )
    ...
    @pytest.mark.asyncio
    async def test_ask_splits_long_response(
        self, chat_cog, mock_interaction, mock_api_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_ask_very_long_response_as_file(
        self, chat_cog, mock_interaction, mock_api_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_ask_empty_after_sanitization(
        self, chat_cog, mock_interaction, mock_api_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_ask_channel_not_allowed(
        self, chat_cog, mock_interaction, sample_config
    ):
