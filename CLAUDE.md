...
- SQLAlchemy 3.1.1 + Alembic migrations (PostgreSQL)
...
- Python 3.12+
- Node.js 20+
- Redis 5.0+ (auto-installed by `start_redis.sh`)
- PostgreSQL 14+ (auto-installed by `start_postgres.sh`)
- CMake + build-essential (auto-installed for Whisper.cpp build)
- FFmpeg (auto-installed for voice processing)
- Ollama (optional, auto-installed for local LLMs)
...
- Provisioning PostgreSQL (install, create database/user, generate credentials)
...
# Run tests in test mode (uses test database)
...
DATABASE_URL         # PostgreSQL connection: postgresql://guaardvark:***@localhost:5432/guaardvark
STORAGE_DIR          # data/
...
- `DATABASE_URL` - PostgreSQL connection string (auto-generated by `start_postgres.sh`)
...
SQLAlchemy models defined in `backend/models.py` (v1.10), backed by PostgreSQL:
...
### PostgreSQL Startup Order (`start_postgres.sh`)

The script must ensure PostgreSQL is **installed and running before** testing the existing `DATABASE_URL`. If the connection test runs before the service is up (e.g., after a reboot), it fails and triggers a full re-provision (new password, user/db creation) requiring `sudo -u postgres`. The correct order is:
1. Install psql if missing (fresh install)
2. Start PostgreSQL service if not running
3. Test existing `DATABASE_URL` — exit early if it works
4. Re-provision only if the connection still fails
