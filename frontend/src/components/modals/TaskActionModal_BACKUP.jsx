...
const TaskActionModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    // Advanced CSV generation parameters (Phase 1)
    client_notes: "",
    competitor_url: "",
    ...
    if (open) {
      ...
      if (isEditMode && taskData) {
        setFormData({
          ...
          target_website: taskData.target_website || "",
          output_filename: taskData.output_filename || "",
          model_name: taskData.model_name || "",
          prompt_rule_id: taskData.prompt_rule_id || null,
          page_count: taskData.page_count || 50,
          auto_start_job: false, // Don't auto-start for existing tasks
          // Advanced parameters with defaults
          client_notes: taskData.client_notes || "",
          competitor_url: taskData.competitor_url || "",
          target_word_count: taskData.target_word_count || 500,
          concurrent_workers: taskData.concurrent_workers || 5,
          batch_size: taskData.batch_size || 25,
        });
      } else {
        ...
        setFormData({
          ...
          client_name: "",
          target_website: "",
          output_filename: `content_${timestamp}.csv`, // Fixed: removed formData.type reference
          model_name: "",
          prompt_rule_id: null,
          page_count: 50,
          auto_start_job: true,
  ...
  const validateForm = () => {
    ...
    if (isFileGeneration) {
      ...
      // Advanced parameter validation (Phase 1) - Only for file generation
      if (formData.target_word_count < 100 || formData.target_word_count > 2000) {
        return "Target word count must be between 100 and 2000";
      }
      if (formData.concurrent_workers < 1 || formData.concurrent_workers > 20) {
        return "Concurrent workers must be between 1 and 20";
      }
      if (formData.batch_size < 5 || formData.batch_size > 100) {
        return "Batch size must be between 5 and 100";
      }
    }
  ...
  const handleCreateTask = async () => {
    ...
    const taskPayload = {
      ...
      target_word_count: isFileGeneration ? formData.target_word_count : null,
      concurrent_workers: isFileGeneration ? formData.concurrent_workers : null,
      batch_size: isFileGeneration ? formData.batch_size : null,
    };
  ...
  // Start file generation job using advanced generateStructuredCSV
  const startFileGenerationJob = async (task) => {
    try {
      // Use the advanced generateStructuredCSV function with all parameters
      const response = await generateStructuredCSV({
        ...
        topics: Array.from({ length: task.page_count }, (_, i) => `${task.client_name} content topic ${i + 1}`),
        num_items: task.page_count,
        target_word_count: task.target_word_count || 500,
        concurrent_workers: task.concurrent_workers || 5,
        batch_size: task.batch_size || 25,
        model_name: task.model_name,
        existing_task_id: task.id, // Link to the database task
        prompt_rule_id: task.prompt_rule_id,
      });

      console.log("TaskActionModal: Advanced CSV generation started successfully:", response);
      return response;
    } catch (error) {
      console.error("TaskActionModal: Advanced CSV generation failed:", error);
      
      // Enhanced error handling with user-friendly messages
      let errorMessage = "CSV generation failed";
      
      if (error.message.includes("timeout")) {
        errorMessage = "CSV generation timed out. Please try again with fewer items or increase timeout.";
          ...
          <Grid container spacing={2}>
            ...
            {(isFileGeneration || isCodeGeneration) && (
              <>
                ...
                {/* Advanced CSV Generation Parameters (Phase 1) - Only for file generation */}
                {isFileGeneration && (
                  <>
                    <Grid item xs={12}>
                      <Typography variant="h6" sx={{ mt: 2, mb: 1, color: 'primary.main' }}>
                        Advanced CSV Generation Settings
                      </Typography>
                    </Grid>
                    ...
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        multiline
                        rows={2}
                        helperText="Business description or context for content generation"
                        placeholder="e.g., Law firm specializing in personal injury cases"
                      />
                    </Grid>
                    ...
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        helperText="Competitor site for content style reference (optional)"
                      />
                    </Grid>
                    ...
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        helperText="Words per content item"
                      />
                    </Grid>
                    ...
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        helperText="Parallel processing threads"
                      />
                    </Grid>
                    ...
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        helperText="Items per processing batch"
                      />
                    </Grid>
                  </>
                )}
